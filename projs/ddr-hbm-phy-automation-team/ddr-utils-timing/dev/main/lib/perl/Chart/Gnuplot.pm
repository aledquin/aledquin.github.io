...
#nolint open<
#nolint backticks
#nolint Modules::ProhibitMultiplePackages
use strict;
use warnings;
use Pod::Usage;
use Data::Dumper;
...
use lib "$RealBin";
use Gnuplot::Util qw(_lineType _pointType _borderCode _fillStyle _copy);
...
#    open(my $GPH, ">>","$self->{_script}") || confess("Can't write $self->{_script}");
#    print $GPH "\nplot $plotString\n";
#    close($GPH);
	my $writeStatus = Util::Misc::write_file("\nplot $plotString\n", "$self->{_script}", '>');
...
sub multiplot
{
    ...
    open(my $PLT, ">>","$self->{_script}") || confess("Can't write $self->{_script}");
...
sub _setChart
{
    ...
    open(my $PLT, ">>","$pltTmp") || confess("Can't write gnuplot script $pltTmp");
    ...
    my @atrr_list  = qw( gnuplot convert encoding imagesize orient bg plotbg timeaxis );
    ...
    foreach my $attr (keys %$self)
    {
        ...
        elsif (!grep {/^$attr$/} @atrr_list && $attr !~ /^_/)
        {
...
sub _reset
{
    my ($self) = @_;
    open(my $PLT, ">>","$self->{_script}") || confess("Can't write $self->{_script}");
	my @attr_list = qw(arrow grid label logscale object parametric);
    foreach my $opt (@{$self->{_sets}})
    {
        ...
        elsif (!grep {/^$opt$/} @attr_list && $opt !~ /tics$/)
        {
            print $PLT "set $opt\n";
        }
    }
...
sub test
{
    ...
    my ($stdout0, $stderr0) = capture { run_system_cmd   ("$gnuplot $pltTmp", "$VERBOSITY");};
...
sub _thawY
{
    ...
    my $fileTmp = $ds->{_data};
    open(my $DATA, ">","$fileTmp") || confess("Can't write data to temp file");
...
sub fit
{
    ...
    if ($paraRef eq 'HASH')
    {
        ...
        open(my $PARA, ">","$parFile") || confess ("Can't write parameter to $parFile");
    ...
    open(my $FIT, ">","$script") || confess("Can't generate script to $script");
    ...
    # Call gnuplot
    my ($stdout1, $stderr1) = capture { run_system_cmd ("gnuplot $script >& /dev/null", "$VERBOSITY");};

    # Read and parse the result file
    open(my $RES,"<", "$result") || confess("Can't read fitting result $result");
