...
set requiredSourceFileList [list pininfoFile equivFile ntMungeCustomConfig]
set ntSourceFileList [list ntConstraintsFile ntExceptionsFile ntPrecheckFile ntPrecheckTopoFile ntPrematchTopoFile ntUserSettingFile]
...
if [info exists scriptArgs(logFile)] {
	...
	proc ntversioncheck {} {
		logMsg "Info: ntversioncheck\n"
		set ::setupStatus(ntversioncheck) 0
		setSetupButtonColor ntversioncheck blue
		set netlistversionDefined  [info exists ::ntVersion]
		if {$netlistversionDefined} {
			set ::setupStatus(ntversioncheck) 1
			logMsg "Error: $::ntVersion\n"
		}

		if $::setupStatus(ntversioncheck) {
			set stat $::ntVersion
			set ::setupStatusInfo(ntversioncheck) "$::ntVersion"
			setSetupButtonColor ntversioncheck red
			.nb.setuptab.f.cnv.win.ntversioncheck.status configure -text $stat
		} else {
			set stat "OK"
			.nb.setuptab.f.cnv.win.ntversioncheck.status configure -text $stat
			setSetupButtonColor ntversioncheck green
		}
		return $::setupStatus(ntversioncheck)
	}
					...
					proc createSourceFile {name textWidget} {
						...
						proc getP4fileInfo {filename} {
							...
							proc processSource {} {
								...
								set ::sourceFileList [concat $::requiredSourceFileList $::ntSourceFileList]
							...
							proc fixupPininfoHeader {hdr} {
								...
								proc createLinkSourceFile {dirName fileVar} {
									...
									if {![info exists $fileVar]} {
										logMsg "Error: $fileVar doesn't exist"
										...
										if [file exists $sourceFile] {
											...
											logMsg "Error: $sourceFile doesn't exist"
													...
													proc processNetlists {} {
														...
														if {$::phase == "pre"} {
															...
															while {[gets $inFile line] >= 0} {
																#POWER_EXTRACT: DEVICE_LAYERS NON_DEVICE_LAYERS_CONLY, only checks first occurance
																if {([regexp {POWER_EXTRACT: (.+)$} $line -> pExt]) && !$checkPE} {
																	if {$pExt == "DEVICE_LAYERS NON_DEVICE_LAYERS_CONLY"} {
																		logMsg "\tOK POWER_EXTRACT is $pExt\n"
																		} else {logMsg "\tWarning:  POWER_EXTRACT is not DEVICE_LAYERS NON_DEVICE_LAYERS_CONLY, please indicate reason. Source: $spfPath\n"}
																		set checkPE 1
																	}
																	...
																	proc mungeLib {runDir srcLib dstLib cfg id} {
																		...
																		if [info exists ::mungeScript] {
																			if [file exists $::mungeScript] {
																				...
																				if {([info exists ::ntEnableCCSN_etm])&&($::ntEnableCCSN_etm == "true")} {
																					if [regexp {(\S+)_pg\.lib} $dstLib dummy srcLibRoot] {
																	...
																	proc runCorner {pvt type force} {
																		...
																		proc createDir {dirName} {
																			...
																			proc createPvtRunScript {dir pvt type} {
																				...
																				if [info exists ::ntExtract_Model_IgnorePort] {
																					set args [set ::ntExtract_Model_IgnorePort]
																					puts $OUT "set  ntExtract_Model_IgnorePort {$args}"
																					} else {puts $OUT "set ntExtract_Model_IgnorePort false"}
																					if [info exists ::ntEnablePbsa_$type] {
																						...
																						if [info exists ::ntEnablePOCV] {
																							set args [set ::ntEnablePOCV]
																							puts $OUT "set ntEnablePOCV {$args}"
																								...
																								proc getRealFile {linkfile} {
																									...
																									proc createMungeConfigs {} {
																										...
																										foreach PVT $::pvtCorners {
																											...
																											if [info exists ::cellArea] {
																												...
																												foreach pin $::groundPins {
																													...
																													puts $CFG "del_pin_attr $pin min_transition
																													del_pin_attr $pin max_capacitance
																													del_pin_attr $pin min_capacitance
																													del_pin_attr $pin rise_capacitance_range
																													del_pin_attr $pin max_transition
																													del_pin_attr $pin fall_capacitance_range
																													"
																												}
																												...
																												puts $CFG "correct_bit_from_to\nsort_arcs\nsort_pins"
																												puts $CFGPG "correct_bit_from_to\nsort_arcs\nsort_pins"
																												close $CFG
																												close $CFGPG
																										...
																										proc setupNTsource {} {
																											...
																											if {[set fp [createSimpleSourceFile "$sourceDir/user_lib_include.tcl"]] != 0} {
																												...
																												if [info exists ::stdCellDbArray] {
																													foreach db [array names ::stdCellDbArray] {
																														lappend l1 "lappend link_path ${db}_\${scCorner}.db"
																														lappend l2 "read_db ${db}_\${scCorner}.db"
																													}
																												}
																											...
																											##  consraints.tcl
																											foreach sf $::ntSourceFileList {
																												createLinkSourceFile $sourceDir $sf
																											}
																										...
																										if [file exists $localConfig] {
																											...
																											foreach step {ntversioncheck validateProject validateLibCell validateP4 processPVT processSource readPininfo readEquiv processSubckts setupNTsource processNetlists setupRundirs setupRunPage setupLibPage setupMergedirs setupMergePage } {
																												setupStatus $step
																											}
																											...
																											if [ntversioncheck] {
																												logMsg "Error: ntVersion set in local config file: $localConfig. Please set ntVersion in global config file.\n"
																												exit
																											}
																											...
																											if [validateProject] {
																												...
																												# nolint Line 1293: E Wrong number of arguments (1) to "if"
																												# nolint Line 1294: E Strange command "
