...
sub Main {
    ...
    if ($opt_help) {
        iprint "Script to generate jitter values \n";
        exit;
    }
    if((@ARGV == 0)) {
    ...
    unlink "./results/jitter_summary.csv";
    ...
    $jitter_cnt = 0;
    my @FILEOUT = ();
    push @FILEOUT, ",Freq,PhaseRJ(ps),PeriodRJ(ps)\n";    
    while ($end == 0) {    
        ...
        run_system_cmd("cd $dir; ./scripts/calc_jitter.pl out.printsnpn0 $BW $fhpf > jitter.dat", "$VERBOSITY");
    
        #combine the jitter results into one spreadsheet
        my @JITFILE_read = read_file("$dir/jitter.dat");
        $str = "$dir";
    
        foreach my $JITFILE (@JITFILE_read) {
            chomp $JITFILE;
            $temp = $JITFILE;
            $temp =~ s/[ ]+//g;
            @fields2 = split /=/, $temp;
