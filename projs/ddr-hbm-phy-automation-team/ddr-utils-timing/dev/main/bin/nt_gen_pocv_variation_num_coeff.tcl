...
set VERBOSITY  0
set STDOUT_LOG ""
set AUTHOR     "DDR Timing"

if {[catch {set RealScript [file tail [file normalize [info script [file link .]]] ] } errorVar2] } {

    set RealScript [file tail [file normalize [info script ]] ]   
}
if {[catch {set RealBin [file dirname [file normalize [info script [file link $RealScript]]] ] } errorVar1] } {
...
lappend auto_path "$RealBin/../lib/tcl/Util/"
...
proc Main {} {
    ...
    puts " Info : Copied  nt_tech.sp  to the new xtor_variations directory \n "
    ...
    foreach instance $instance_list {
        ...
        for {set x 0} {$x < [array size ptransistor]} {incr x} {
            ...
            while {$status == "running"} {
                ...
                puts -nonewline "##Info : Waiting for $jobsList jobs to complete, Total:$i, Pending Jobs:$j, Running Jobs:$k  \n"
            ...
            if { [file exists ./$__coeff_file] }  {
                ...
                puts " POCV_XTOR_Info : __coeff_file $__coeff_file created \n"
                puts " POCV_XTOR_Info : Done executing rupocv_create_variation_coeff for  ${PVT}_${instance}_$ptransistor($x)_$ntransistor($x)_$lengthtransistor($x)_$__vdd_voltage volts successfully \n"
            } else {
                puts " POCV_XTOR_ERROR : __coeff_file : $__coeff_file  NOT created  \n "
            }
    ...
    puts "\n POCV_XTOR_INFO : END OF THE LOOP for $PVT corner ; Execute run_pocv_merge to merge the results obtained \n "
    puts "\n  ########################################### end ####################################################################################\n"
    puts "\n  ########################################### end ####################################################################################\n"
    ...
    ###################################################### loop end ############# exited   #######################  loop end ###########################


    puts "Info : All the  COMBINATIONS AS PER THE GIVEN USERFILE covered for $cellName for corner $PVT . About to merge the variation results obtained as a single file using run_pocv_merge  in  xtor_variations directory \n "
