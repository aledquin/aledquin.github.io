...
#		pub::unset_obj_attr $cellObj "state_function"
		pub::unset_obj_attr $cellObj "x_function"
	    }
	}
    }
}
...
##  Reads all libs for a given location and macro into memory for mods
    ...
    foreach pvt $active_pvts {
	set path [pwd]
	puts "puts ${path}/$loc/models/liberty/cells/${macro}_${pvt}.lib.gz"
	set libName "${path}/$loc/models/liberty/cells/${macro}_${pvt}.lib.gz"
	exec chmod 777 -R "${path}/$loc/models/liberty/cells"
...
proc copyPOCVsidefile {sloc dloc macro prefix suffix} {
##  copy created pocv sidefile from sloc to dloc  
    ...
    set sourcepath "$sloc"
    set destinationpath "$dloc"
    
    if {[file exists $sourcepath] == 0} {puts "\n sourcelocation $sloc not found to copy pocv side file \n "}
    if {[file exists $destinationpath] == 0} {exec mkdir $dloc}
    
    foreach pvt $active_pvts {
#    set mypwd [$PWD]
#    puts "\n sourcepath $sloc  ::::::: destinationpath $dloc  \n    pwd : $PWD  ::  [PWD]   ::: $mypwd \n "
    foreach file [glob -nocomplain $sourcepath/${pvt}.pocv] {file copy -force $file $destinationpath/${macro}_$::STACK\_${pvt}.pocv}	
    }
}
