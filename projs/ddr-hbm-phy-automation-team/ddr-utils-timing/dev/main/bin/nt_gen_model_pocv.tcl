...
if [info exists INPUTS_VDDQ_VDD2H] {
    if {[llength $INPUTS_VDDQ_VDD2H] > 0} {
	set_model_input_transition_indexes -max -nominal [expr $etm_tr_nominal/0.6] $tr_idx $INPUTS_VDDQ_VDD2H -rail_voltage $VDDQ_VDD2H_val
	set_model_input_transition_indexes -min -nominal [expr $etm_tr_nominal/0.6] $tr_idx $INPUTS_VDDQ_VDD2H -rail_voltage $VDDQ_VDD2H_val
    }
...
if [info exists loadRangeSlew] {
    foreach slewID [array names loadRangeSlew] {
	   ...
	   if [info exists INPUTS_VAA] {
	    ...
	    if [info exists INPUTS_VDDQ_VDD2H] {
    		if {! $found_rail_voltage} { 
            	    foreach pin_source $INPUTS_VDDQ_VDD2H { 
	        	if {$pin_config == $pin_source} { 
			    set rail_voltage $VDDQ_VDD2H_val
	    		    set found_rail_voltage 1
        		    break
	        	}
        	    }
