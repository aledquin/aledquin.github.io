...
if ($hspice_ver == "NA") then
    if ($MODE == "int") then
        ...
        nt_shell -f nt_gen_pocv_variation_num.tcl > POCV_generation.rpt
        rm -f statusPOCV_generation_Running
        touch statusPOCV_generation_Complete
    
#####     rm -f statusQueued
#####    touch statusRunning
#####    nt_shell -f run_nt_etm.tcl > report.rpt
#####    rm -f statusRunning
#####    touch statusComplete
    ...
    if ($MODE == "batch") then
        unsetenv LS_COLORS
         ...
         # qsub -P bnormal -A $QUEUE -l mem_free=$MEM,h_vmem=$VMEM $EXTRASGEARGS -V -cwd -b y -N NT -e NT.err -o NT.out "rm -f statusQueued; touch statusRunning; nt_gen_pocv_variation_num.tcl > pocv_generation.log; rm -f statusRunning; touch statusComplete"
    
        qsub -P bnormal -A $QUEUE -l mem_free=$MEM,h_vmem=$VMEM $EXTRASGEARGS -V -cwd -b y -N NT -e POCV_generation.err -o POCV_generation.out "rm -f statusPOCV_generation_Queued; touch statusPOCV_generation_Running; nt_gen_pocv_variation_num.tcl > POCV_generation.log; rm -f statusPOCV_generation_Running; touch statusPOCV_generation_Complete"
    ...
    if ($MODE == "int") then
        rm -f statusPOCV_generation_Queued
        touch statusPOCV_generation_Running
        nt_shell -f nt_gen_pocv_variation_num.tcl $hspice_ver > POCV_generation.rpt
        rm -f statusPOCV_generation_Running
        touch statusPOCV_generation_Complete
