...
if ($MODE == "int") then
    rm -f statusQueued
    touch statusRunning
    nt_shell -f run_nt_internal.tcl > report.rpt
    rm -f statusRunning
...
if ($MODE == "batch") then
    unsetenv LS_COLORS
    qsub -P bnormal -A $QUEUE -l mem_free=$MEM,h_vmem=$VMEM $EXTRASGEARGS -V -cwd -b y -N NT -e NT.err -o NT.out "rm -f statusQueued; touch statusRunning; nt_shell -f run_nt_internal.tcl > timing.log; rm -f statusRunning; touch statusComplete"
endif
