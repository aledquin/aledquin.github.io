...
set INOUT_VDD2_status 0
set INOUT_VDD2H_status 0

#########################################
##global settings
############################################
#source ${PROJ_HOME}/design/timing/nt/ntFiles/nt_global_settings.tcl
set script_dirname [file dirname [file normalize [info script]]]
...
######## Trace paths ##################################
#######################################################
#note : -keep_paths_within 0.05 is added to -pbsa option in boons run_nt_internal.tcl during path tracing --> srisowm 29-jan-2018 

if $ntEnablePbsa {
 if $ntEnablePOCV {
     #trace_paths -pbsa -keep_paths_within .005 -pocv (-keep_paths_within .005  )
     trace_paths -pbsa  -pocv
     } else {
     trace_paths -pbsa
     }
