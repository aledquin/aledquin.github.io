...
sub Main {
	...
	if((@ARGV == 0)) {
        eprint "Script to generate all course tdt clock skew\n";
		exit;
    ...
    $idx = 0;
    foreach my $countrval_sweepcsv (@countrval_sweep) {
        chomp $countrval_sweepcsv;
        @fields = split /,/, $countrval_sweepcsv;
        if ($fields[1] =~ /^[0-9-]/) {
            $temp = int($fields[1]);
    ...
    if ($rise_or_fall eq "1") {
        @Param_xclkrdly = read_file("Param_xclkrdly/clkr_clkv8r_skew_sweep.csv");
    } else {
        @Param_xclkrdly = read_file("Param_xclkrdly/clkr_clkv8f_skew_sweep.csv");
    }
    $idx = 0;
    
    foreach my $clkr (@Param_xclkrdly) {
        chomp $clkr;
        @fields = split /,/, $clkr;
        if ($fields[1] =~ /^[0-9-]/) {
            $temp = $fields[1];
    ...
    my @clktdc = read_file("Param_xclkrdly/clkr_clktdc_skew_sweep.csv");
    $idx = 0;
    foreach my $clkr_clktdc (@clktdc) {
        chomp $clkr_clktdc;
        @fields = split /,/, $clkr_clktdc;
        if ($fields[1] =~ /^[0-9-]/) {
            $temp = $fields[1];
            $clktdcval[$idx] = $temp; 
            $idx++;
