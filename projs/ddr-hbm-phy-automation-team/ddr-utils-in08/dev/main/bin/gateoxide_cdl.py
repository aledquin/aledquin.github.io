...
# Gives cumulative width and length of devices in .cdl files
# nolint main
import os
import subprocess
import collections
...
worksheet_summary.write('D3', 'cumulative width (mm)', cell_format_border_gray)

worksheet_detail.write('A1', 'Transistor Count Detail', cell_format_border_bold)
worksheet_detail.merge_range('B1:G1', str(os.getcwd()), cell_format_border)
...
worksheet_detail.write('E3', 'width', cell_format_border_gray)
worksheet_detail.write('F3', 'count', cell_format_border_gray)
worksheet_detail.write('G3', 'cumulative width (mm)', cell_format_border_gray)
...
for macro in sorted(final_count.keys()):
    ...
    for inner_key in sorted(final_count[macro].keys()):
        ...
        worksheet_detail.write(row_detail, col + 5, final_count[macro][inner_key], cell_format_border)
        worksheet_detail.write(row_detail, col + 6, float(((float(split[2]) * 30 - 22) * float(final_count[macro][inner_key])) / 1000000), cell_format_border)
        row_detail += 1
        if (split[0], split[1]) not in width_sum.keys():
            width_sum[(split[0], split[1])] = float(((float(split[2]) * 30 - 22) * float(final_count[macro][inner_key])) / 1000000)
        else:
            width_sum[(split[0], split[1])] += float(((float(split[2]) * 30 - 22) * float(final_count[macro][inner_key])) / 1000000)
