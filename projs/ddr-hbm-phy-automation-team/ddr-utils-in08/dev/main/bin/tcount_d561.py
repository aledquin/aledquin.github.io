...
# Transistor count script for .cdl files modified for d561
import os
import subprocess
import collections
import re
                ...
                for line in data:
                    ...
                    if line.lower().startswith('.subckt'):
                        ...
                        if any(x in line.split() for x in device_list):
                            ...
                            # device
                            devlist = ['ptgmv', 'ntgmv', 'plvt', 'nlvt', 'n', 'p', 'ntg', 'nlplvt', 'plplvt']
                            devlist_cap = ['ddnwpw', 'b88djnesd_prim', 'b88djpesd_prim']
                            ...
                            elif any(x in line.split() for x in devlist_cap) and 'area=' in line and 'pj=' in line and 'm=' in line:
                                # print('in')
                                device = next(element for element in line.split() if element in devlist_cap)
                                # device = re.search("([a-z0-9-_.+]+cap_18 )", line, re.IGNORECASE).group(1).strip()
                                lr = re.search(".*area=([a-z0-9-_.+]+).*", line, re.IGNORECASE).group(1)
                                nfin = re.search(".*pj=([a-z0-9-_.+]+).*", line, re.IGNORECASE).group(1)
                                m = re.search(".*m=([a-z0-9-_.+]+).*", line, re.IGNORECASE).group(1)
                                if m[-1] == 'k':
                                    m = int(float(m[0:-1]) * 1000)
                                # print int(m)
