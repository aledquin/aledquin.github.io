...
proc placeDevice {fp type device pList idx} {
	...
	set devParams [regsub -all "=$pName" $devParams "=$pVal"]
	incr i
    }

    ##  BOZO:  Need something to distinguish MOS and other devices
    puts $fp "${type}dut_$idx $devNodes $device $devParams"
}
...
proc genFixedResTable {type device} {
    ##  Just a single value.
    set g $::meas(g_0)
    set tableHdr [list $type $device]
    return [list $tableHdr [list $g]]
}

proc genMosSimulation {id device corner} {
...
#    set simPath $simPath
    ...
    set i 0
    foreach pList $::paramPermutes {
	placeDevice $OUT "M" $device $pList $i
	set offset [getParam $pList OFFSET]
	placeMeasurementSource $OUT $i $offset
	incr i
    }

    if [info exists ::hspiceOptionsFile] {puts $OUT ".include \"$::hspiceOptionsFile\""}
    ...
    if [info exists ::hspiceSourceCommands] {
	foreach l $::hspiceSourceCommands {puts $CSH $l}
    }
#    puts $CSH "setenv INTEL_PDK              \${udecadrep}/fab/f124-Intel/10nm/GP/FDK_msip/pdk/pdk756_r1.2.3_snps"
#    puts $CSH "setenv hspice_lib_models      \${INTEL_PDK}/cmi/hspice/cmi/lnx86/64bit"
#    puts $CSH "setenv PDMI_LIB               \${INTEL_PDK}/cmi/hspice/pdmi/lnx86/64bit/pdmi.so"
...
proc genFixedResSimulation {id device corner supplyV} {
...
proc genMosTables {type device pvt supply paramList rowParam} {

    set supplyV [supplyVoltage $pvt $supply]
    genMosSimulation $type $device $pvt
...
proc genFixedResTables {type device pvt supply} {

    set supplyV [supplyVoltage $pvt $supply]
    genFixedResSimulation $type $device $pvt $supplyV
    set table [genFixedResTable $type $device]
    lappend ::deviceResTables($device) $table
    set ::deviceList($device) 1
}
...
proc charRes3tFixed {device PVT supply} {
    ##  For a fixed dimension resistor
    set ::deviceType($device) RES

    set_device_nodes $device {T1 T2 B}
    set_device_params $device {}
    ...
    genFixedResTables Gsd $device $PVT $supply
}

## Procedure to run a standard 4 terminal mosfet.
proc charMos {device type PVT supply w l diffA diffP} {
    ...
    set_device_params $device {w=W l=L m=1 as=AS ad=AD ps=PS pd=PD pes=W ped=W nf=1 sndi=0 sidn=0 sndn=0 sidi=W}
    set vSupply [supplyVoltage $PVT $supply]
    ...
    ##  Single l,w used for diff sims
    set oneL [lindex $l 0]
    set oneNfin [lindex $w 0]

    ##  R sims
    reset_sim
    set_node_supply [list S $VS] [list G $VD] [list B $VS] 
    set_measurement_node D
    set_param [list W $w] [list L $l] {PS 0} {AS 0} {PD 0} {AD 0}
    genResTables Gsd $device $PVT $supply {L} W

    ##  Cgate sims
    reset_sim
    set_node_supply [list S $VS] [list D $supply] [list B $VS]
    set_measurement_node G
    set_param [list OFFSET [list 0 $vSupply]] [list W $w] [list L $l] {PS 0} {AS 0} {PD 0} {AD 0}
    genMosTables Cg $device $PVT $supply {OFFSET L} W
    ...
    set_measurement_node D
    set_param [list OFFSET [list 0 $vSupply]] [list W $w] [list L $l] {PS 0} {AS 0} {PD 0} {AD 0}
    genMosTables Csd $device $PVT $supply {OFFSET L} W

    ##  Diff area (ad) sims
    reset_sim
    set_node_supply [list S $VS] [list G $VS] [list B $VS] 
    set_measurement_node D
    set_param [list OFFSET [list 0 $vSupply]] [list W $oneNfin] [list L $oneL] {PS 0} {AS 0} {PD 0} [list AD $diffA]
    genMosTables Cad $device $PVT $supply {OFFSET} AD

    ##  Diff periphery (pd) sims
    reset_sim
    set_node_supply [list S $VS] [list G $VS] [list B $VS] 
    set_measurement_node D
    set_param [list OFFSET [list 0 $vSupply]] [list W $oneNfin] [list L $oneL] {PS 0} {AS 0} {AD 0} [list PD $diffP]
    genMosTables Cpd $device $PVT $supply {OFFSET} PD

    ##  Diff area (as) sims
    reset_sim
    set_node_supply [list S $VS] [list G $VS] [list B $VS] 
    set_measurement_node S
    set_param [list OFFSET [list 0 $vSupply]] [list W $oneNfin] [list L $oneL] {PS 0} {AD 0} {PD 0} [list AS $diffA]
    genMosTables Cas $device $PVT $supply {OFFSET} AS

    ##  Diff periphery (ps) sims
    reset_sim
    set_node_supply [list S $VS] [list G $VS] [list B $VS] 
    set_measurement_node S
    set_param [list OFFSET [list 0 $vSupply]] [list W $oneNfin] [list L $oneL] {PD 0} {AS 0} {AD 0} [list PS $diffP]
    genMosTables Cps $device $PVT $supply {OFFSET} PS

}


# nolint Line 169: E Wrong