#!/bin/tcsh -f
#
# https://jira.internal.synopsys.com/browse/P10020416-38528
#
# Customer used ddr-ckt-rel/2022.10
source /global/etc/modules/3.1.6/init/tcsh
#module unload ddr-ckt-rel
#module load   ddr-ckt-rel/2022.10
#/remote/cad-rep/msip/tools/Shelltools/ddr-ckt-rel/2022.10/bin/
...
# p4 sync -f //depot/products/lpddr5x_ddr5_phy/lp5x/project/d931-lpddr5x-tsmc3eff-12/ckt/rel/dwc_lpddr5xphycover_acx2_top_ew/1.00a/...
#
set p4ws               = p4_ws
set p4client           = msip_cd_${USER}
set p4_root            = `realpath ~/${p4ws}`
set project_family_cad = "lpddr5x"
set project_family     = "lpddr5x_ddr5_phy/lp5x"
set macro              = "dwc_lpddr5xphycover_acx2_top_ew"
set project            = "d931-lpddr5x-tsmc3eff-12"
set project_cad        = "d931-lpddr5x-tsmc3eff-12"
set release_cad        = "rel1.00_cktpcs"
#    <--- this is set by legalRelease.txt  'rel' setting
#             Required to edit the legalRelease.txt file; under CAD area so 'rel' says 1.00a
#             Also needed to modify alphaHLDepotRelease.pm to point to this modified legalRelease.txt file
...
#set exe_dir            = "/remote/cad-rep/msip/tools/Shelltools/ddr-ckt-rel/2022.10/bin/"
#set exe_dir            = ""
set script             = ${exe_dir}alphaHLDepotRelPinCheck
set script_options     = "-macros ${macro} -p ${proj_spec}"
set run_prefix         = ""
set tmp_legal_rel_dir  = "/tmp/tests/data/alphaHLDepotRelPinCheck"
mkdir -p $tmp_legal_rel_dir
set tmp_legal_release  = "${tmp_legal_rel_dir}/${project_family_cad}.${project}.${release_cad}.legalRelease.txt"
cp -f legalRelease.txt  $tmp_legal_release

set test_options       = "" #" -debug 6 -verbosity 5"
...
FIND_MISSING:
set the_pincheck = "${p4_root}/depot/${p4_path}/${macro}/${release_ver}/macro/${macro}.pincheck"
if ( ! -e $the_pincheck ) then
    echo "**** can not find $the_pincheck ****"
else
    echo "Looking for 'missing in' and -I- Pin"
    grep -P -- "missing in|-I- Pin" $the_pincheck 
    echo "Looking for -W- warnings"
    grep -P -- '-W-' $the_pincheck
endif

echo "Pincheck file: $the_pincheck"
