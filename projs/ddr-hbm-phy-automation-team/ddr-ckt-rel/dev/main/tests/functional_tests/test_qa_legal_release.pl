...
sub Main(){
    my $scriptName = "qa_legal_release.pl";
    my %t = ( 
        '01'=>{
            'description' => "Make sure the script runs",
        ...
        '02'=>{
            'description' => "Make sure the script passes with just -help",
            'testScript'  =>"$RealScript",
            'testDataDir' =>"$RealBin/../data",
            'scriptDir'   =>"$RealBin/../../bin",
            'scriptName'  =>"$scriptName",
        ...
        '03'=>{
            'description' => "Make sure the script fails when unable to find the file",
            'testScript'  =>"$RealScript",
            'testDataDir' =>"$RealBin/../data",
            'scriptDir'   =>"$RealBin/../../bin",
            'scriptName'  =>"$scriptName",
            'product'     =>"lpddr5x",
            'project'     =>"d991-lpddr5x-tsmc3eff-12",
            'release'     =>"rel1.00_cktpcs",
            'extraArgs'   =>"",
            'expectFail'  => TRUE,
        },
    ...
    foreach my $tstnum (sort keys %t) {
       ...
       if ( $status != 0 ){
            if ( $href_args->{'expectFail'} ){
                # this was expected to fail
                if ( $stdout !~ m/none of these files exist/i ){
                    # we expected to see a specific string somewhere in the
                    # output.  but we didn't see it; so fail this test.
