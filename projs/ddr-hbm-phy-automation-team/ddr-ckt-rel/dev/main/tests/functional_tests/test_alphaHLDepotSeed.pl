...
sub Main(){
    my $scriptName = "alphaHLDepotSeed";
    my %t = ( 
        '01'=>{
        ...
        '05'=>{
            'description' => "dwc_lpddr5xphy_txrxac_ew -hspice -ibis",
            'testScript'  =>"$RealScript",
            'testDataDir' =>"$RealBin/../data",
            'scriptDir'   =>"$RealBin/../../bin",
            'scriptName'  =>"$scriptName",
...
#        if ( $tstnum eq "01"){
#            $ENV{'DDR_DA_COMPARATOR_DIFF'} = 1;
#        }else{
#            $ENV{'DDR_DA_COMPARATOR_DIFF'} = 0;
#        }
