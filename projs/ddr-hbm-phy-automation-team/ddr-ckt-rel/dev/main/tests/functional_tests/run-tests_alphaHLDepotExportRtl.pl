...
sub Main(){
    ...
    my ($tests_list, $exit_val) = 
        run_system_cmd( "ls -1 $RealBin/test_alphaHLDepotExportRtl*.pl", $VERBOSITY);
    ...
    # ExportRtl doesn't allow -p4_ws cmd line option
    $optWorkspace = "";
    ...
    foreach my $test ( @tests ){
        ...
        my $cmd =  "$test $optWorkspace $optCoverage $optCleanup"
                        ."$optVerbosity $optDebug";
        iprint( "Running cmd: '$cmd'\n" );
