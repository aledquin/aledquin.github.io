...
sub Main(){
    ...
    my ($tests_list, $exit_val) = 
        run_system_cmd( "ls -1 $RealBin/test_alphaPinCheck.pl", $VERBOSITY);
    #print "Tests => $tests_list\n";
    ...
    my @tests = split('\n', $tests_list);
    #print "Tests => " .scalar(Dumper \@tests) ."\n" ;
    ...
    $optTestNum   = "-testnum $opt_tnum"    if ( $opt_tnum );
    ...
    foreach my $test ( @tests ){
        ...
        my ($stdout, $status) = 
            run_system_cmd( "$test $optWorkspace $optCoverage $optCleanup"
                ."$optTestNum $optVerbosity $optDebug", $VERBOSITY );
