...
} else {set logFile "./ntManager.log"}
...
proc refreshProjName {} {
	...
	set projectNameGlob [glob -nocomplain -types d "*"]
	cd $cwd
...
proc refreshReleaseName {} {
	...
	if {[info exists ::projectType] && [info exists ::projectName]} {
		...
		set releaseNameGlob [glob -nocomplain -types d "*"]
		foreach r [lsort $releaseNameGlob] {
			if [file exists $r/cad/project.env] {
				.nb.projtab.f.cnv.win.release.list.lb insert end $r
			}
		}
		cd $cwd
    	}
...
proc refreshMetalStack {} {
	...
	set stackNameGlob [glob -nocomplain -types d "*"]
...
proc validatePOCVinfo {} {
    ...
    if [info exists ::ntEnablePOCV_etm] {
		...
		logMsg "set ntEnablePOCVvalidate $args9"
			set stat "OK with ntEnablePOCV_etm $args9 "
    } else {
      	logMsg  "set ntEnablePOCVvalidate false"
...
#    puts $SCR "module load lc/2014.09-SP5"
...
#	    puts "\t$cell"
	...
	if {!$isStdCell} {
		...
		if {$srcLib != ""} {
		    ...
		    logMsg "SUCCESS: lib FOUND for $bbox $pvt  and is $dstLib --> $srcLib\n"
		} else {
...
#	    puts "\t$cell"
	...
	if {!$isStdCell} {
		...
		if {$srcpocvsidefile != ""} {
		    ...
		    logMsg "SUCCESS: pocvsidefile FOUND for $bbox $pvt    and is $dstpocvsidefile --> $srcpocvsidefile\n"
		} else {
		    ...
		    if {$srcpocvsidefile != ""} {
			createSimpleLink $dstpocvsidefile $srcpocvsidefile
			logMsg "Warning:  Found only non-metalStack-specific pocvsidefile for ${bbox}_$pvt\n"
		    } else {
			logMsg "Error:  Could not find pocv side file for $bbox $pvt\n"
			## Quick hack to id cases where libs are missing.
			set ::pocvsidefilesOK($bbox) 0
			lappend ::setupStatusInfo(processPOCVSubckts) "pocvsidefile Not Found $bbox"
			set status 0
		    }
		}
	    }
...
set status 1
lappend ::setupStatusInfo(processPOCVSubckts) "bbox pocvsidefile Not required for CREATING XTOR PARAMS with nt"
set ::setupStatus(processPOCVSubckts) $status
    if $status {
	.nb.setuptab.f.cnv.win.processPOCVSubckts.status configure -text "OK for CREATING XTOR PARAMS"

    } else {
	.nb.setuptab.f.cnv.win.processPOCVSubckts.status configure -text "Failed non-pocv flow"
    }
    return $status
...
##########################sow###############################
proc create_POCV_var_coeff {pvt force} {

    ##  Runs a corner, type
#    logMsg "Info: Creating POCV var coeff file $pvt \n"
    set runDir "./timing/Run_${pvt}_etm"
      set runscript "nt_create_pocv_var_coeff.csh"
    set cmd "./$runscript"
    set status [getPocvStatus $pvt]
	...
	logMsg "Info:  Skipped Creating POCV var coeff file for $pvt\n"
	return 
    }
	...
	if [regexp {Your job (\d+) .* has been submitted} $output dummy jobID] {
	    .nb.pocvtab.f.cnv.win.fr.$pvt.status config -text "Job $jobID"
	    set ::jobIDarray("${pvt}") $jobID
	    createEmptyFile "$runDir/createPOCVstatusQueued"
	} else {
	    createEmptyFile "$runDir/createPOCVstatusQueuedFailed"
	}
    } else {puts "Error:   $runDir/$runscript for creating pocv var coeff file does not exist\n"}
}
...
proc killCornerPocv {pvt} {
    ##  Kill a corner,
#    logMsg "Info: Running $pvt\n"

    set runDir "./timing/Run_${pvt}"
    if [info exists ::jobIDarray("${pvt}")] {
	set status [getPocvStatus $pvt]
	if {($status == "Queued") || ($status == "Running")} {
	    set jobID $::jobIDarray("${pvt}")
	    logMsg "Info:  Killing job $jobID\n"
	    set msg [exec qdel $jobID]
	    logMsg "Info:  $msg\n"
	    deleteFiles "$runDir/status*"
	    set SF [open "$runDir/statusKilled" w]
	    close $SF
	    .nb.pocvtab.f.cnv.win.fr.$pvt.status config -text "Killed"
	}
#	logMsg "Info:  Status = $status\n"
		    
    } else {
	logMsg "Warning:  Job for $pvt has not been started\n"
    }
...
proc setupPocvPageRow {pvt} {
if {([info exists ::ntEnablePOCV_etm])&&($::ntEnablePOCV_etm == "true")} {

    set fname .nb.pocvtab.f.cnv.win.fr.$pvt
    frame $fname
    pack [label $fname.label -text "$pvt:" -width 29 -font MyDefaultFont] -side left
    # pack [entry $fname.netlist -width 60] -side left
    # set netlist $::netlistList($pvt)
    # $fname.netlist insert end $netlist
    #foreach runType $::typeList {
	pack [button $fname.varPOCV -text "create_POCV_var_coeff" -width 20 -command "create_POCV_var_coeff $pvt 1" -font MyDefaultFont] -side left
	pack [button $fname.kill -text "Kill" -width 10 -command "killCornerPocv $pvt" -font MyDefaultFont] -side left
	pack [label $fname.status -text "" -width 10 -font MyDefaultFont] -side left
	
    #}
   # puts ".nb.pocvtab.f.cnv.win.fr.$pvt"
    return $fname
 ...
 set fname .nb.pocvtab.f.cnv.win.fr.$pvt
    frame $fname
    pack [label $fname.label -text "Running w/o pocv " -width 20 -font MyDefaultFont] -side left
    # pack [entry $fname.netlist -width 60] -side left
    # set netlist $::netlistList($pvt)
    # $fname.netlist insert end $netlist
    #foreach runType $::typeList {
...
proc createAll_POCV_var_coeff {} {foreach pvt $::pvtCorners {create_POCV_var_coeff $pvt 0}}
proc killAllCorners {type} {foreach pvt $::pvtCorners {killCorner $pvt $type}}
proc killAllCornersPocv {} {foreach pvt $::pvtCorners {killCorner $pvt}}
...
proc getPocvStatus {pvt } {

    set runDir "./timing/Run_${pvt}_etm"
    if [info exists ::runjobIDarray("${pvt}")] {set jobID $::runjobIDarray("${pvt}") } else {set jobID ""}
    set POCVstatusFiles [glob -nocomplain "$runDir/POCVstatus*"]
    set NPOCVstatus [llength $POCVstatusFiles]
    if {$NPOCVstatus == 0} {
	set POCVstatus "none"
    } elseif {$NPOCVstatus == 1} {
	regexp {status(.*)} $POCVstatusFiles dummy POCVstatus
    } else {
	logMsg "Warning:  Multiple status files for $pvt/  {$POCVstatusFiles}\n"
	set POCVstatus "Unknown"
    }
...
proc checkPocvStatus {} {

    foreach pvt $::pvtCorners {
	foreach type $::typeList {
	    set status [getPocvStatus $pvt]
	    .nb.pocvtab.f.cnv.win.fr.$pvt.status config -text $status
	    ##  Refresh existence of etm libs.
	    refreshLibPageRow $pvt
	}
    }
...
if {([info exists ::ntEnablePOCV_etm])&&($::ntEnablePOCV_etm == "true")} {
    ...
    logMsg "Info:  setting up POCV var_coeff_Page\n"
    setSetupButtonColor setupPocvPage blue
    if [winfo exists .nb.pocvtab.f.cnv.win.fr] {destroy .nb.pocvtab.f.cnv.win.fr}
    frame .nb.pocvtab.f.cnv.win.fr

    set fname .nb.pocvtab.f.cnv.win.fr.runAll
    frame $fname
    pack [label $fname.label -text "All PVT:" -width 19 -font MyDefaultFont] -side left
    pack [button .nb.pocvtab.f.cnv.win.fr.buttonCheck -text "CheckStatus" -command "checkPocvStatus" -font MyDefaultFont] -side top
    pack [button $fname.create_POCV_var_coeff -text " createAllPOCV_var_coeff " -width 20 -command "createAll_POCV_var_coeff  " -font MyDefaultFont] -side left
    pack [button $fname.killAll -text "KillAll" -width 10 -command "killAllCornersPocv" -font MyDefaultFont] -side left
    #pack [label $fname.status_$runType -text "" -width 10 -font MyDefaultFont] -side left
    pack .nb.pocvtab.f.cnv.win.fr.runAll -side top
    pack .nb.pocvtab.f.cnv.win.fr

    foreach pvt $::pvtCorners {
	pack [setupPocvPageRow $pvt] -side top
    }
    
    set ::setupStatus(setupPocvPage) 1
    set ::setupStatusInfo(setupPocvPage) ""
    .nb.setuptab.f.cnv.win.setupPocvPage.status configure -text "OK WITH POCV "
    return 1

} else {

    logMsg "Info: not setting up POCV var_coeff_Page\n"
    setSetupButtonColor setupPocvPage blue
    if [winfo exists .nb.pocvtab.f.cnv.win.fr] {destroy .nb.pocvtab.f.cnv.win.fr}
    frame .nb.pocvtab.f.cnv.win.fr
   ...
   # pack [button $fname.killAll -text "KillAll" -width 10 -command "killAllCornersPocv" -font MyDefaultFont] -side left
    ...
    set ::setupStatus(setupPocvPage) 1
    set ::setupStatusInfo(setupPocvPage) ""
    .nb.setuptab.f.cnv.win.setupPocvPage.status configure -text "OK W/O POCV "
return 1
}
}
proc setupRunPage {} {
...
proc createPvtRunScript {dir pvt type} {
    ...
    puts $OUT "set PROJ_HOME $env(MSIP_PROJ_ROOT)/$::projectType/$::projectName/$::releaseName"
...
if [info exists ::sisVersion] {
	...
	puts $OUT "set SISVersion $args445"
        ...
        puts $OUT "set SISVersion siliconsmart/2017.06-SP3 "
	}
...
proc createPvtPocvScript {dir pvt type} {
    global env

    set OUT [open "$dir/Pocvsetup.tcl" w]
    #puts $OUT "set PROJ_HOME /remote/us01home52/ektas/scripts/alpha"
    puts $OUT "set PROJ_HOME $env(MSIP_PROJ_ROOT)/$::projectType/$::projectName/$::releaseName"
    puts $OUT "set PVT $pvt"
    puts $OUT "set runType $type"
 ...
 ########################################################### cell name top
   ...
   if [info exists ::pocvcellName ] {
 	set args40 [set ::pocvcellName]
	puts $OUT "set cellName $args40"
	puts $OUT "set cornerval $pvt "
       	}
    
    if [info exists ::pocvk ] {
 	set args404 [set ::pocvk]
	puts $OUT "set k _etm"
	}
    ...
    if [info exists ::pocvnetlist] {
 	set args30 [set ::pocvnetlist]
	puts $OUT "set netlist $args30"
    } else {puts $OUT "#set netlist false"}
    
    
    if [info exists ::pocvequiv] {
 	  set args319 [set ::pocvequiv]
	  puts $OUT "set equiv $args319"
    } else {
    
         if [info exists ::equivFile] {
 	 set args3012 [set ::equivFile]
	 puts $OUT "set equiv $args3012"
         } else {puts $OUT "#set equiv false  as equivFile and pocvequiv variables are not set in alphaNT.config"}
    
    }
    
    
    if [info exists ::pocvcdl] {
 	set args3111 [set ::pocvcdl]
	puts $OUT "set cdl $args3111"
    } else {
    
        if [info exists ::pocvnetlist] {
 	set args3011 [set ::pocvnetlist]
	puts $OUT "set cdl $args3011"
        } else {puts $OUT "#set cdl false  as pocvnetlist and pocvcdl are not set in alphaNT.config"}
    
    }
    
    
    if [info exists ::userfile] {
 	set args3199 [set ::userfile]
	puts $OUT "set userfile $args3199"
    } else {puts $OUT "#set userfile false"}
    
    
    
    if [info exists ::pocvspf] {
 	set args315 [set ::pocvspf]
	puts $OUT "set spf $args315"
    } else {puts $OUT "#set spf false"}
    


    if [info exists ::pocvhspicemodel] {
 	set args319 [set ::pocvhspicemodel]
	puts $OUT "set hspicemodel $args319"
    } else {puts $OUT "#set hspicemodel false"}
    

    if [info exists ::pocv__scriptdir] {
 	set args311 [set ::pocv__scriptdir]
	puts $OUT "set __scriptdir $args311"
    } else {puts $OUT "#set __scriptdir false"}
...
proc setupRundirs {} {
	...
	foreach pvt $::pvtCorners {
		...
		if [file exists $::scriptDir/nt_create_pocv_var_coeff_$type.csh] {
		    createSimpleLink $dir/nt_create_pocv_var_coeff.csh $::scriptDir/nt_create_pocv_var_coeff_$type.csh
		} else {
		    createSimpleLink $dir/nt_create_pocv_var_coeff.csh $::scriptDir/nt_create_pocv_var_coeff_$majorType.csh
		}
		
		
		
		if [file exists $::scriptDir/nt_gen_pocv_variation_num.tcl ] {
		    createSimpleLink $dir/nt_gen_pocv_variation_num.tcl  $::scriptDir/nt_gen_pocv_variation_num.tcl 
		} else {
			puts "Error script is not present"
		}
		...
		set vstep [format "%0.3f" [expr 0.1*$maxSupply]]
...
proc setupPocvdirs {} {
	...
	foreach pvt $::pvtCorners {
		...
		##  Create the pvt_setup script.  This is where the cell/pvt info is kept.
		createPvtRunScript $dir $pvt $type
		createPvtPocvScript $dir $pvt $type
		##  The run-NT tcl script
		if [file exists $::scriptDir/run_nt_$type.tcl] {
		    createSimpleLink $dir/run_nt_$type.tcl $::scriptDir/run_nt_$type.tcl 
		} else {
		...
		if [file exists $::scriptDir/nt_gen_pocv_variation_num.tcl ] {
			...
			puts "Error nt_gen_pocv_variation_num.tcl script is not present in used ntFiles directory"
		}
		
		if [file exists $::scriptDir/run_pocv_merge ] {
		    createSimpleLink $dir/run_pocv_merge  $::scriptDir/run_pocv_merge 
		} else {
			puts "Error run_pocv_merge script is not present in used ntFiles directory"
		}
		...
		if [file exists $::scriptDir/run_sis_sidefile_pocv.csh ] {
			...
			puts "Error run_sis_sidefile_pocv.csh script is not present in used ntFiles directory"
		}
		
		
		
		
		if [file exists $::scriptDir/run_get_macro_list ] {
		    createSimpleLink $dir/run_get_macro_list  $::scriptDir/run_get_macro_list
		} else {
			puts "Error run_get_macro_list script is not present in used ntFiles directory"
		}
		
		if {([info exists ::ntEnablePOCV_etm])&&($::ntEnablePOCV_etm == "true")} {
			if [file exists $::scriptDir/gen_pocvSideFile.tcl ] {
			    createSimpleLink $dir/gen_pocvSideFile.tcl  $::scriptDir/gen_pocvSideFile.tcl
			} else {
				puts "Error gen_pocvSideFile.tcl script is not present in used ntFiles directory"
			}
		} else {
...
.nb add [frame .nb.pocvtab] -text "Pocv"
...
foreach step {validateProject validateLibCell validateP4 processPVT validatePOCVinfo processSource readPininfo readEquiv processSubckts setupNTsource processNetlists setupRundirs setupPocvPage setupRunPage setupLibPage setupMergedirs setupMergePage } {
    setupStatus $step
}
...
# nolint Main
# nolint Line 2975: E Wrong number of arguments
