...
###############################################################################
#
# Name    : alphaNoise_deepupdated.pl
# Author  : N/A
# Date    : N/A
# Purpose : N/A
#
...
use constant VARTYPE_NUMERIC => 0;
use constant VARTYPE_STRING  => 1;


my $g_LOG;  # file handle for logs
my $RPT;  # file handle 
my $g_strict_leading_dot;
my $g_strict_wildcard_slash;
my @g_Powers;
my @g_Grounds;
my $g_href_DomainVoltage;
...
##  g_noiseVars can be changed using the "set" command.
my $g_noiseVars = {
    'defaultGup' => {
...
my $gTheStaticClass;
my $gTheDefaultClass;
my $gMinMosgroupFanoutDomain;
my %g_pininfo;
my $href_deviceTables;
my $g_linebuf;
my %g_uncharDeviceWarning;
my $g_fileDone;
...
########  MAIN FOLLOWS  ##############
sub Main(){
    my @copy_args = @ARGV; # save original script arguments

    if (!@ARGV) {
        print_usage(1); # exists with error status 1
    }

    $g_strict_leading_dot    = 0;
    $g_strict_wildcard_slash = 0;

    ##  Master list of power/ground nodes.  Preloaded with defaults.  
    ##  These are interpreted as regular expressions.
    @g_Powers = qw( VDD );
    $g_href_DomainVoltage = {
        "VDD" => 1.0
    };
    @g_Grounds = qw( VSS 0 );
    ...
    if ($help) {
        print_usage(0); # automaticaly terminates exit(0)
    }    

    unless ( $DEBUG || $opt_nousage){
        utils__script_usage_statistics( $RealScript, $VERSION, \@copy_args);
    }
    open($g_LOG, ">", $logFile) || confess "Error: Cannot open log file $logFile\n"; # nolint open>
    ...
    #print "Powers = {@g_Powers}\n";
    #print "Grounds = {@g_Grounds}\n";
    ...
    print "Run complete.  See $logFile for details\n";
...
sub cleanupDangles {
    ...
    #    print "!!!  $nodeName:  \"$g\" \"$m\" \"$p\"\n";
        if (!$n->{MOSGROUP}      && !$n->{ISPIN} && 
            !$n->{VIRTUALPOWER}  && !$n->{VIRTUALGROUND} ) {
            ##  Scrub dangling gate nodes. 
            ##  These are typically pode gates that will usually appear in 
            ##  the netlist as tied to a rail through a large resistor (ignored)
...
sub validateDomain {
    my $domain = shift;
    return (defined $g_href_DomainVoltage->{$domain})
}
...
sub report_coupling_func {
    ...
    if (!$out) {
    my $macro = $g_noiseVars->{macro}->{VALUE};
    if (!$macro) {$out = "coupling.report"}
    else {$out = "${macro}_coupling.report"}
    }

    if (!open($RPT, ">", $out)) { # nolint open>
...
sub gen_noise_model_func {
    ...
    open my $NOISE, ">", $out || confess "Failed to open file for write '$out': $!\n"; # nolint open>
    ...
    foreach my $nodeName (keys %nodeData) {
        ...
        if ($n->{ISPIN} && !($n->{ISPOWER} || $n->{ISGROUND})) {
            ...
            my $r_high = ($n->{GdownCoup}) ? 1e-3/$n->{GdownCoup} : $g_noiseVars->{infiniteR}->{VALUE};
            my $r_low = ($n->{GupCoup}) ? 1e-3/$n->{GupCoup} : $g_noiseVars->{infiniteR}->{VALUE};
            my $class = $n->{CLASS};
    ...
    foreach my $nodeName (@nodeList ){
        ...
        if ($details) {
            if ($rec->{ISPOWER} || $rec->{ISGROUND}) {
                if ($rec->{ISPOWER}) {
                    my $v = $g_href_DomainVoltage->{$nodeName};
...
sub define_limit_func {
    ...
    if (defined $fanoutClass) {
        if (validateNodeClass($fanoutClass)){
            push @fanoutList, $fanoutClass;
        } else {
            @fanoutList = keys %{$nodeClasses};
        }
        foreach my $nodeClass (@ARGV) {
            foreach my $fc (@fanoutList ){
                if (defined $clkLimit ){
                    $nodeClasses->{$nodeClass}->{LIMITS}->{$fc}->{CLKLIMIT} = $clkLimit;
                }
                if (defined $sigLimit ){
                    $nodeClasses->{$nodeClass}->{LIMITS}->{$fc}->{SIGLIMIT} = $sigLimit;
                }
                $nodeClasses->{$nodeClass}->{LIMITS}->{$fc}->{WARN} = $warn;
            }
        }
    }
} # end define_limit_func
...
sub genFanouts {
    ...
    foreach my $outputNode (@{$m->{OUTPUTS}}) {
        ...
        $outputNode->{CLASS} = $gTheDefaultClass;
...
#        print "\t$outputNodeDomain\n";
        my $outputNodeDomainVoltage = $g_href_DomainVoltage->{$outputNodeDomain};
...
#            print "$inputNode->{FANOUT}->{LIMIT}\n";
    ...
    foreach my $outputNode (@{$m->{OUTPUTS}}) {
        ...
        if (!(defined $limit)) {
        logMsg(LOG_WARNING, "Using default fanout for node $outputNode->{NAME}\n");
        ##  Fanout is undefined.  Define it as default.
        $limitPct = ($outputNode->{ISCLK}) ? 
            $nodeClasses->{$outputNodeClass}->{LIMITS}->{$gTheDefaultClass}->{CLKLIMIT} : 
            $nodeClasses->{$outputNodeClass}->{LIMITS}->{$gTheDefaultClass}->{SIGLIMIT};
        my @dh = @{$outputNode->{DOMAIN}};
        my $minDomainName = minDomain(\@dh);  #Default to lowest domain.
        my $domainV = $g_href_DomainVoltage->{$minDomainName};
...
#        print "\t!!!  $nnn  limit=$limit limitPct=$limitPct  {$minDomainName}  {$domainV}\n";
        ...
        $outputNode->{FANOUT}->{CLASS} = $gTheDefaultClass;
        }
        ...
        $gMinMosgroupFanoutDomain = $outputNode->{FANOUT}->{DOMAIN};
        }
    }
    ...
    foreach my $n (@{$m->{NODES}}) {
        ...
        $n->{FANOUT}->{DOMAIN} = $gMinMosgroupFanoutDomain;
...
sub finalNodeClass {
    ...
    foreach my $nodeName (keys %nodeData ){
        ...
        my $n = $nodeData{$nodeName};
        ##  Use pininfo data, if available.  Otherwise, infer direction 
        ## from device connections.
        if ($n->{ISPIN}) {
            my $pi;
            if ($pi = $g_pininfo{$nodeName}) {
        #       print "Info:  Pininfo found for $nodeName \\$pi->{DIR}\\, 
        #              \\$pi->{RELATED_POWER}\\\n";
                $n->{DIR}         = $pi->{DIR};
                $n->{DOMAIN}      = ();
                $n->{DOMAIN}->[0] = $pi->{RELATED_POWER};
                $n->{AGGVOLTAGE}  = $g_href_DomainVoltage->{$pi->{RELATED_POWER}};
            }else{
                if (defined ($n->{MOSGROUP})) {
                    if ($n->{GATEFANOUT} ){
                        $n->{DIR} = "io";
                    }  ##  Has paths to power/ground, but also gates.  
                       ##  Not clear, but assume IO
                    else { 
                        $n->{DIR} = "output"; ##  Has paths to power/ground, 
                                              ##  but not gates.  Output
                    }
                }
                else {
                    ## No paths to power/ground.
                    if ($n->{GATEFANOUT} ){ 
                        $n->{DIR} = "input";  ##  Connects to gate(s).  Input
                    } else { 
                        $n->{DIR} = "Unknown"; # No device connections at all.
                    }  
                }
            }
        }
        ...
        if (($n->{GupCoup}>0) && ($n->{GdownCoup}>0)) {
            $n->{CLASS} = $gTheStaticClass;
        }

        ##  Last resort:  Undefined class/domain for node,  use default
        if (!(defined $n->{CLASS}) && !$n->{ISPOWER} && !$n->{ISGROUND}) {
            $n->{CLASS} = $gTheDefaultClass;
            logMsg(LOG_WARNING, "Using default class \"$gTheDefaultClass\" for node $n->{NAME}\n");
        }

        my @dl = @{$n->{DOMAIN}};
        if ((@dl == 0) && !$n->{ISPOWER} && !$n->{ISGROUND}) {
            push @{$n->{DOMAIN}}, $g_noiseVars->{defaultDomain}->{VALUE};
            logMsg(LOG_WARNING, "Using default domain \"$g_noiseVars->{defaultDomain}->{VALUE}\" for node $n->{NAME}\n");
        }
    }
} # end findNodeClass
...
#    foreach my $n (@{$m->{OUTPUTS}}) {print "\t\t$n->{NAME}\n"};
#    print "\tDevices:\n";
...
#            print "\tInternal\n"
    ...
    if ($nOutputs == 0) {$m->{CLASS} = $gTheStaticClass}   ##  Special case for mosgroups with no outputs.  Usually dummy devices.
    ##  Mark all input fanouts according to the mosgroup class.
    ##  Can't set fanouts yet; node class may be changed by command.
    $i++;
    }  ##  End mosgroup loop
} # end processMosgroups
...
sub addNodeG {
    ...
    #print "addNodeG:  $node->{NAME}, G=$G, dir=$dir, refG=$refG, incG=$incrG\n";
    my $incrG = $G;
    # original line if ($refG != 0) {$incrG = 1/((1/$G)+(1/$incrG))}  ##  Add series R, then reconvert to G.
    if ($refG != 0 && $G != 0) {$incrG = 1/((1/$G)+(1/$incrG))}  ##  Add series R, then reconvert to G.
...
sub setAggVoltage {
    ...
    foreach my $nodeName (sort(keys %nodeData)) {
        ...
        if (@domains == 1) {
            ...
            $nodeData{$nodeName}->{AGGVOLTAGE} = $g_href_DomainVoltage->{$domains[0]};
            $nodeData{$nodeName}->{VICVOLTAGE} = $g_href_DomainVoltage->{$domains[0]};
            next;
        }
        my $high = 0;
        my $low = 999;
        ...
        foreach my $d (@domains) {
            $high =  ($g_href_DomainVoltage->{$d} > $high) ? $g_href_DomainVoltage->{$d} : $high;
            $low =  ($g_href_DomainVoltage->{$d} < $low) ? $g_href_DomainVoltage->{$d} : $low;
        }
...
#    foreach my $aggNode (keys %$ignoreAggressor) {
#    foreach my $vicNode (keys %{$ignoreAggressor->{$aggNode}}) {
#    }
#    }
...
sub calcNoise {
    foreach my $nodeName (sort(keys %nodeData) ){
        ...
        my $fanoutDomainVoltage = $g_href_DomainVoltage->{$fanoutDomain};
    ...
    #    if (!(defined $fanoutLimit)) {print "Warning:  Undefined fanout for $nodeName\n"; return}
        ...
        my $noiseLH = 0;
        my $noiseHL = 0;
        my $gup =   (defined $rec->{GupCoup}) ? $rec->{GupCoup} : $g_noiseVars->{defaultGup}->{VALUE};
        my $gdown = (defined $rec->{GdownCoup}) ? $rec->{GdownCoup} : $g_noiseVars->{defaultGdown}->{VALUE};
        foreach my $agg (@{$rec->{CCOUPLE}}) {
            ...
            my $aggTrise =  (defined $aggRec->{TRISE}) ? $aggRec->{TRISE} : $g_noiseVars->{defaultAggTrise}->{VALUE};
            my $aggTfall = (defined $aggRec->{TFALL}) ? $aggRec->{TFALL} : $g_noiseVars->{defaultAggTfall}->{VALUE};
...
#    open(my $datFH, "<", $dataFile);
    my @datFH = read_file( $dataFile);
    my $lineNum = 0;
    foreach my $line (@datFH) {
        $line =~ s/\#.*//;   ##  Uncomment
        $lineNum++;
...
sub read_tech_func {
    ...
    if (-r $techFile) {
        ...
        do $techFile;
#        print Dumper($href_deviceTables);
    }  else {
...
sub set_domain_voltage_func {
    ...
    foreach my $t (@$toks) {
    if ($t =~ /^(\S+)=(\S+)$/) {
        logMsg(LOG_INFO, "Setting $1 domain voltage to $2\n");
        $g_href_DomainVoltage->{$1} = $2;
    } else {
...
sub setDriverDevice {
    ...
    if (defined ($href_deviceTables->{$deviceName})) {
    my $wvar = lc $href_deviceTables->{$deviceName}->{WVAR};
    my $lvar = lc $href_deviceTables->{$deviceName}->{LVAR};
...
sub set_func {
    ...
    my $varName = $toks->[0];
    my $varValue = $toks->[1];
    my $type = $g_noiseVars->{$varName}->{TYPE};
    if (defined $type) {
    if ($type == VARTYPE_NUMERIC){
        $g_noiseVars->{$varName}->{VALUE} = SpiceNumToReal($varValue);
    } else {
        $g_noiseVars->{$varName}->{VALUE} = $varValue};
    } else {
...
sub set_powers_func {
    ...
    @g_Powers = ();  ##  Flush the defaults.
    foreach my $supply (@$toks) {push @g_Powers, $supply}
}
...
sub minDomain {
    ...
    if ($g_href_DomainVoltage->{$d} < $minV) {$minV = $g_href_DomainVoltage->{$d}; $minD = $d}
    }
    return $minD;
}
...
sub updateFanoutLimit {
    ...
    if (!$class) {
    $class = $gTheDefaultClass;
    }

    my $domainVoltage = $g_href_DomainVoltage->{$domain};
...
sub set_grounds_func {
    ...
    @g_Grounds = qw( ^0$ );  ##  Flush the defaults, but always keep the node "0".
    foreach my $ground (@$toks) {push @g_Grounds, $ground}
}
...
sub getNodeCaps {
    ...
    my $x = $href_deviceTables->{$device}->{NODECAPS}->{$node};
    return $x;
}
...
sub read_netlist_func
{
    ...
    open(my $NET, "<", $infile) || confess "Error: Cannot open $infile\n"; # nolint open<
    logMsg(LOG_INFO, "Reading $infile\n");
    while (my $line = GetLine($NET) ){
        ...
        if ($tok1)
        {
            ...
            if ($id eq "X") {ProcessInstance(\@tokens )}
            elsif ($id eq "C") {ProcessCap(\@tokens )}
            elsif ($id eq "R") {ProcessRes(\@tokens )}
            elsif ($id eq "M") {ProcessMos(\@tokens )}
            elsif ($id eq "D") {ProcessDiode(\@tokens )}
...
sub GetLine
{
    ...
    ## $g_linebuf should hold the pre-fetched next non-continuation line.
    if (!$g_linebuf) {
        $g_linebuf = <$fh>;  ##  Read first line of file.
    }  
    while ($line = <$fh> ){
        ##$g_linebuf =~ s/^\s*(.*)\s*$/$1/g;
        if (substr($line, 0, 1) eq "+"){
            ...
            chomp $g_linebuf;
            $g_linebuf .= substr($line, 1);
        }else{
            ##  Non-continuation.  Return $g_linebuf
            my $rline = $g_linebuf;
            $g_linebuf = $line;
            return $rline;
        }
...
sub parseGenericDevice {
    ...
    foreach my $nodeName (@$nodes) {
        ...
        push @$nodeRec, $n;
        $n->{DEVCONNECT} = 1;   ## Tag that a given node connects to an active
                                ## device.  Used later to filter out dangles.
    }
    ...
    my $devNodes = undef;
    if (defined $href_deviceTables->{$devName}) {
        $devNodes = $href_deviceTables->{$devName}->{NODES};
        my $type = $href_deviceTables->{$devName}->{TYPE};
        $rec->{TYPE} = $type;
        if ($type eq "MOS") {
            $rec->{ISMOS} = 1;
            $rec->{MOSTYPE} = uc $href_deviceTables->{$devName}->{MOSTYPE};  ## Generally P or N
            $rec->{WVAR} = $href_deviceTables->{$devName}->{WVAR};  ##  The param associated with width, typically W or NFIN
            $rec->{LVAR} = $href_deviceTables->{$devName}->{LVAR};  ##  The param associated with length, typically L.
    ...
    #        print "resistor g = $rec->{G}\n";
        ...
        if (defined $g_uncharDeviceWarning{$devName}) {
            ##  Warning already issued
        } else {
            $g_uncharDeviceWarning{$devName} = 1;
...
sub processNodeDeviceCaps {
    ...
    my $nodeCaps = $href_deviceTables->{$devName}->{NODECAPS}->{$devNode};
    my ($cap0,$cap1) = genDeviceCaps($devNode, $nodeCaps, $devRec, $devName);
...
sub genDeviceResSingle {
    ...
    my $res;
    my $table = $href_deviceTables->{$devName}->{RES}->{$resTable};
    my $tableData = $href_deviceTables->{$devName}->{RES}->{$resTable}->{DATA};
...
#    print "Info:  Getting device G for $devName.  $rowVar=$rowVarVal: ";

    if (@$tableParams == 0) {
    ##  No table really.  Just a single value
    ##  BOZO:  Add check for a real single-entry table.
    $res = $tableData->[0]->[0] * $rowVarVal;
    } else {
...
sub genDeviceCapSingle {
    ...
    my $cap;
    my $table = $href_deviceTables->{$devName}->{CAPS}->{$capTable};
    my $tableData = $href_deviceTables->{$devName}->{CAPS}->{$capTable}->{DATA};
    my $tableParams = $table->{tableParams};
    my $rowVar = lc $table->{rowVar};
    my $rowVarVal = 1;
    if ($rowVar ne "") {
...
#    print "-genDeviceCapSingle: $capTable, $devName {@$tableParams}\n";
    ...
    $cap = ($cap > $g_noiseVars->{capIgnoreThresh}->{VALUE}) ? $cap : 0;
...
sub isPower {
    my $nodeName = shift;
    foreach my $powerPatt (@g_Powers) {if ($nodeName =~ /^$powerPatt$/) {return 1}}
    return 0;
}

sub isGround {
    my $nodeName = shift;
    foreach my $groundPatt (@g_Grounds) {if ($nodeName =~ /^$groundPatt$/) {return 1}}
    return 0;
}
...
#    print "Interp:  x=$x, x1,y1=$x1,$y1, x2,y2=$x2,$y2 --> $y\n";
    return $y;
}
...
sub read_pininfo_func
{
    ...
    foreach my $csv_file (@$toks) {
        if (!(-r $csv_file)) {
            logMsg(LOG_WARNING, "CSV file \"$csv_file\" cannot be read\n"); next;
        }
        logMsg(LOG_INFO, "Reading $csv_file\n"); 
        
        #open my $CSV, "<", $csv_file;
        my @arrayCSV = read_file( $csv_file);
        #my $line = <$CSV>;  ##  Read header
        my $line = shift @arrayCSV;
        chomp $line;
        ...
        my $i = 0;
        foreach my $hval (@headers ){
            $hdr{$hval} = $i++;
        }  ## Build a hash to look up column numbers
        my $area;
#        while ($line = <$CSV>)
        foreach my $line (@arrayCSV) {
            chomp $line;
            $line =~ s/\s+//g;
            my @t = split(/,/, $line);
            ...
            ##  Add new ones
            foreach my $pinname (bitBlast($name)) {$g_pininfo{$pinname} = $rec}
        }
#        close $CSV;
    }
...
# print the help info and then exit with the given status
# Note: pod2usage will exit.
#
sub print_usage($) {
    my $exit_status = shift;

    my $verbose_level = 2 ; # entire manual page

    my $msg = "Current script path: $RealBin\n";
    pod2usage({
       -message => $msg,
       -exitval => $exit_status,
