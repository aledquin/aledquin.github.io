...
set uniqueprefix PCSQAUSER_hbm3_d763-hbm3-v2-tsmc3eff-12-ew_rel1.00_EWHardened_
...
proc pcsqa21e_msg { file msgtext } {
	...
	if { [string equal -length [string length $pcs] $pcs $file] == 1 } {
   	     	puts "SNPS_ERROR  : $msgtext"
	} else {
   	     	puts "SNPS_WARNING: $msgtext"
	}
}
...
proc checkpermissions { typefile file parentfile } {
	...
	if { [string compare -length [string length $cadrepproj] $cadrepproj $file] == 0 || [string compare -length [string length $projcad ] $projcad $file] == 0 } {
		...
		if { $fileowner != "csadmin" } {
   	     		pcsqa21e_msg $file "(PCSQA-2.1e1 REMOTESITE) $typefile in $parentfile is not owned by csadmin: $file ($fileowner:$filegroup)"
   		}
		incr checks
		if { $filegroupid != $sg_pdks } {
   	     		pcsqa21e_msg $file "(PCSQA-2.1e1 REMOTESITE) $typefile in $parentfile file does not belong to $sg_pdks (sg_pdks) group ID: $file ($fileowner:$filegroupid\($filegroup\))"
   		}
		incr checks
   		if { [matchpermissions $permissions $gomw_gopr_permissions] == 0 } {
   	     		pcsqa21e_msg $file "(PCSQA-2.1e1 REMOTESITE) $typefile in $parentfile file has not $gomw_gopr_permissions_txt permissions: $file (x$gopermissions)"
   		}
	} else {
		if { [string compare -length [string length $cadrepfab] $cadrepfab $file] == 0 } {
			incr checks
			if { $fileowner != "csadmin" } {
   	     			pcsqa21e_msg $file "(PCSQA-2.1e2 REMOTESITE) $typefile in $parentfile is not owned by csadmin: $file ($fileowner:$filegroup)"
   			}
			incr checks
			if { $filegroupid != $sg_pdks } {
   	     			pcsqa21e_msg $file "(PCSQA-2.1e2 REMOTESITE) $typefile in $parentfile file does not belong to $sg_pdks (sg_pdks) group ID: $file ($fileowner:$filegroupid\($filegroup\))"
   			}
			incr checks
   			if { [matchpermissions $permissions $gomw_gpr_permissions] == 0 } {
   	     			pcsqa21e_msg $file "(PCSQA-2.1e2 REMOTESITE) $typefile in $parentfile file has not $gomw_gpr_permissions_txt permissions: $file (x$gopermissions)"
   			}
		} else {
			if { [string compare -length [string length $cadrepmsip] $cadrepmsip $file] == 0 } {
				...
				if { [string compare -length [string length $cadrepmsippdklegacy] $cadrepmsippdklegacy $file] == 0 ||
					...
					incr checks
					if { $fileowner != "root" } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile is not owned by root: $file ($fileowner:$filegroup)"
   					}
					incr checks
					if { $filegroupid != $root } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile file does not belong to $root (root) group ID: $file ($fileowner:$filegroupid\($filegroup\))"
   					}
					incr checks
					if { [matchpermissions $permissions $gomw_gopr_permissions] == 0 } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile file has not $gomw_gopr_permissions_txt permissions: $file (x$gopermissions)"
   					}
				} else {
					incr checks
					if { $fileowner != "csadmin" } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile is not owned by csadmin: $file ($fileowner:$filegroup)"
   					}
					incr checks
					if { $filegroupid != $wwcad } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile file does not belong to $wwcad (wwcad) group ID: $file ($fileowner:$filegroupid\($filegroup\))"
   					}
					incr checks
					if { [matchpermissions $permissions $gomw_gpr_permissions] == 0 } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e3 REMOTESITE) $typefile in $parentfile file has not $gomw_gpr_permissions_txt permissions: $file (x$gopermissions)"
   					}
				}
			} else {
				if { [string compare -length [string length "/global"] "/global" $file] == 0 } {
					...
					if { $fileowner != "root" && $fileowner != "ids_cm" } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e4 REMOTESITE) $typefile in $parentfile is not owned by root or ids_cm: $file ($fileowner:$filegroup)"
   					}
					...
					incr checks
    					if { [matchpermissions $permissions "xxxxxx1xx"] == 0 } {
   	     					pcsqa21e_msg $file "(PCSQA-2.1e4 REMOTESITE) $typefile in $parentfile file has not o+r permissions: $file (xx$opermissions)"
   					}
				} else {
					if { [string compare -length [string length "dummy"] "dummy" $file] != 0 &&
						...
						incr checks
						if { $fileowner != "csadmin" } {
   	     						pcsqa21e_msg $file "(PCSQA-2.1e5 REMOTESITE) $typefile in $parentfile is not owned by csadmin: $file ($fileowner:$filegroup)"
   						}
						incr checks
						if { $filegroupid != $sg_pdks } {
   	     						pcsqa21e_msg $file "(PCSQA-2.1e5 REMOTESITE) $typefile in $parentfile file does not belong to $sg_pdks (sg_pdks) group ID: $file ($fileowner:$filegroupid\($filegroup\))"
   						}
						set permissions [string range $permissions [expr [string length $permissions] - 3] [expr [string length $permissions] - 1]]
						incr checks
   						if { [matchpermissions $permissions $gomw_gpr_permissions] == 0 } {
   	     						pcsqa21e_msg $file "(PCSQA-2.1e5 REMOTESITE) $typefile in $parentfile file has not $gomw_gpr_permissions_txt permissions: $file ($permissions)"
   						}
					}
				}
			}
...
set pcs $::env(PCSQA_PROJ_ROOT)/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened
set msipprojroot $ude3proj
if { [file isdirectory $pcs] == 0 } {
	set pcs [file normalize $getp4contentwork/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened]
	set msipprojroot [file normalize $getp4contentwork]
}

if { $siteid == "REMOTESITE" && "REMOTESITE" != "de02" && "REMOTESITE" != "us01" && "REMOTESITE" != "in01" } {
	set pcsdesign $pcs/design
	if { [file isdirectory $pcsdesign] && [file type $pcsdesign] != "link" } {
		puts "SNPS_WARNING: PCSQA is not applicable for site REMOTESITE, because design directory in PCS is no link"
		exit 0
	}
} 
...
if { [file exists $projectenv] } {
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_PARENTCHILD_TYPE | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_parentchild_type
...
set proj_home $ude3proj/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened
if { [file isdirectory $proj_home] == 0 } {
	set proj_home $pcs	
}
...
set refccs "/remote/proj/cad/c269-tsmc3eff-1.2v/rel3.2.0"
...
if { $refccs != "" } {
    ...
    if { [file exists $projectenv] } {
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_CAD_PROJ_NAME | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_cad_proj_name
	if { $msip_cad_proj_name == "" } {
		set msip_cad_proj_name MSIP_CAD_PROJ_NAME
	}
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_CAD_REL_NAME  | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_cad_rel_name
...
if { $parentpcs != "" } {
    ...
    if { [file exists $projectenv] } {
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_PARENT_PCS_PRODUCT_NAME | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_parent_pcs_product_name
	if { $msip_parent_pcs_product_name == "" } {
		set msip_parent_pcs_product_name  MSIP_PARENT_PCS_PRODUCT_NAME	
	}
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_PARENT_PCS_PROJ_NAME    | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_parent_pcs_proj_name
	if { $msip_parent_pcs_proj_name == "" } {
		set msip_parent_pcs_proj_name     MSIP_PARENT_PCS_PROJ_NAME
	}
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_PARENT_PCS_REL_NAME     | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_parent_pcs_rel_name
...
if { $refccs != "" && [file exists $refccs/design/project.env] } {
	catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e module | grep -e customdesigner >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e unsetenv | grep -e MSIP_CD_VERSION | sed -e 's/unsetenv\\s*MSIP_CD_VERSION\[^\\n\]*/module unload customdesigner/g' >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | sed -e 's/setenv\\s*MSIP_CD_VERSION\\s*/module load customdesigner\\//g' -e 's/\"//g' >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | wc -l" } msip_cd_version_lines
	if { $msip_cd_version_lines > 0 } {
		catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | awk '{print \$3}' | tail -1 | sed -e 's/\"//g'" } msip_cd_version
	}
}
if { [file exists $projectenv] } {
	catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e module | grep -e customdesigner >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e unsetenv | grep -e MSIP_CD_VERSION | sed -e 's/unsetenv\\s*MSIP_CD_VERSION\[^\\n\]*/module unload customdesigner/g' >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | sed -e 's/setenv\\s*MSIP_CD_VERSION\\s*/module load customdesigner\\//g' -e 's/\"//g' >> ${uniqueprefix}loadmodules" } dummy
	catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | wc -l" } msip_cd_version_lines
	if { $msip_cd_version_lines > 0 } {
		catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIP_CD_VERSION | awk '{print \$3}' | tail -1 | sed -e 's/\"//g'" } msip_cd_version
	}
}
...
if { [file exists $projectenv] } {
	...
	if { $msip_parentchild_type != "CHILD_RnD_LIBS" && $msip_parentchild_type != "PARENT" && $msip_parent_pcs_product_name != "" && $msip_parent_pcs_proj_name != "" && $msip_parent_pcs_rel_name != "" } {
		...
		catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e setenv | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | sed -e 's#MSIP_PROJ_ROOT#$ude3proj#g' | $macroreplace -e 's/MSIP_PARENT_PCS_PRODUCT_NAME/$msip_parent_pcs_product_name/g' -e 's/MSIP_PARENT_PCS_PROJ_NAME/$msip_parent_pcs_proj_name/g' -e 's/MSIP_PARENT_PCS_REL_NAME/$msip_parent_pcs_rel_name/g'" } parentprojectenv
		...
		if { $parentprojectenv != $refparentprojectenv } {
			if { $msip_parentchild_type == "CHILD_CAD_SETUP" } {
				puts "SNPS_ERROR  : (PCSQA-2.1.1e REMOTESITE) First line of PCS project.env file is not source of a Parent PCS project.env: $parentprojectenv"
			} else {
				puts "SNPS_ERROR  : (PCSQA-2.1a REMOTESITE) First line of PCS project.env file is not source of a Parent PCS project.env: $parentprojectenv"
			}
		}
	} else {
	...
	set cad_proj_home $udeproj/cad/$msip_cad_proj_name/$msip_cad_rel_name
	set ccsreplace    "-e 's#CAD_PROJ_HOME#$cad_proj_home#g' -e 's#MSIP_CAD_REL_NAME#$msip_cad_rel_name#g' -e 's#MSIP_CAD_PROJ_NAME#$msip_cad_proj_name#g' -e 's#PROJ_HOME#$proj_home#g'"
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e setenv | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | $macroreplace $ccsreplace" } ccsprojectenv
	...
	if { $ccsprojectenv != $refccsprojectenv } {
		...
		if { $ccsmatch == 0 } {
			catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e setenv | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | $macroreplace $ccsreplace" } firstline
			puts "SNPS_ERROR  : (PCSQA-2.1a REMOTESITE) First line of PCS project.env file is not source of CCS project.env: $firstline"
		} else {
			## redefine refccs
			## Support of setenv MSIP_CAD_PROJ_NAME and MSIP_CAD_REL_NAME in project.env 
			catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_CAD_PROJ_NAME | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_cad_proj_name
			catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_CAD_REL_NAME  | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_cad_rel_name
			catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e setenv | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | sed -e 's/\${MSIP_CAD_PROJ_NAME}/$msip_cad_proj_name/g' -e 's/\$MSIP_CAD_PROJ_NAME/$msip_cad_proj_name/g' -e 's/\${MSIP_CAD_REL_NAME}/$msip_cad_rel_name/g' -e 's/\$MSIP_CAD_REL_NAME/$msip_cad_rel_name/g'" } ccsprojectenv
		...
		incr checks
		catch { exec $exec_cmd -c "grep ^\[\[:blank:\]\]*setenv\[\[:blank:\]\]*RUN_DIR_ROOT $projectenv | grep -v ^# | wc -l" } rundirrootavailable
		if { $rundirrootavailable > 0 } {
			catch { exec $exec_cmd -c "grep ^\[\[:blank:\]\]*setenv\[\[:blank:\]\]*RUN_DIR_ROOT $projectenv | grep -v ^# | tail -1 | awk '{print \$3}' | sed -e 's/\"//g' -e 's/\{//g' -e 's/\}//g'" } rundirrootvalue
...
##			}
			if { $rundirrootvalue != $refvalue } {
				puts "SNPS_ERROR  : (PCSQA-2.1v REMOTESITE) Value of redefinition of RUN_DIR_ROOT in PCS project.env file is '$rundirrootvalue' and not '$refvalue'"		
			}
		} 
##		Not mandatory any more since PCS QA spec 14.1
##		else {
##			puts "SNPS_ERROR  : (PCSQA-2.1v REMOTESITE) No redefinition of RUN_DIR_ROOT in PCS project.env file is available"		
##		}
	}
	set msip_cad_rel_name  [file tail $refccs]
	set msip_cad_proj_name [file tail [file dirname $refccs]]
	...
	if { [file exists $pcs/design/project.env] } {
		...
		if { $projectenvtarget != "" } {
			...
			if { $projectenvtarget != [file normalize $projectenv] } {
				puts "SNPS_ERROR  : (PCSQA-2.1l REMOTESITE) design/project.env is no link to cad/project.env"
			}
		} else {
			puts "SNPS_ERROR  : (PCSQA-2.1l REMOTESITE) design/project.env is no link to cad/project.env"
		}		
	...
	puts "SNPS_ERROR  : (PCSQA-1 REMOTESITE) No project.env file found at $projectenv"
} 
...
proc checkdefines { file parentfile usemetalstack } {
	...
	catch { exec sh -c "cat $file | grep -v ^# | grep DEFINE | grep -v -e UNDEFINE -e PROJ_P4_ROOT | awk '{print \$3}' | $macroreplace $ccsreplace $parentpcsreplace" } defines
	...
	foreach define $defines {
		...
		if { [file isdirectory $define] == 0 } {
			...
			if { [string first METAL_STACK $define] == -1 } {
				pcsqa21e_msg $define "(PCSQA-2.1e REMOTESITE) Refered library in $parentfile file does not exists: $define"
			} else {
				...
				foreach envtcl $envtcls {
					...
					if { [file isdirectory $metaldefine] == 0 } {
						pcsqa21e_msg $metaldefine "(PCSQA-2.1e REMOTESITE) Refered library in $parentfile file does not exists: $metaldefine"
					} else {
...
proc checkincludes { file parentfile usemetalstack } {
	...
	catch { exec sh -c "cat $file | grep -v ^# | grep INCLUDE     | grep -v -e PROJ_P4_ROOT -e SOFTINCLUDE | awk '{print \$2}' | $macroreplace $ccsreplace $parentpcsreplace" } includes
	catch { exec sh -c "cat $file | grep -v ^# | grep SOFTINCLUDE | grep -v -e PROJ_P4_ROOT                | awk '{print \$2}' | $macroreplace $ccsreplace $parentpcsreplace" } softincludes
	...
	foreach include $includeslist {
		...
		if { [file exists $include] == 0 } {
			...
			if { [string first METAL_STACK $include] == -1 } {
				if { $soft == 0 } {
					pcsqa21e_msg $include "(PCSQA-2.1e REMOTESITE) Include file in $parentfile file does not exists: $include"
				}
			} else {
				if { $usemetalstack == "all" } {
					set envtcls [glob -nocomplain $pcs/cad/*/env.tcl]
				...
				foreach envtcl $envtcls {
					...
					if { [file exists $metalinclude] == 0 } {
						if { $soft == 0 } {
							pcsqa21e_msg $metalinclude "(PCSQA-2.1e REMOTESITE) Include file in $parentfile file does not exists: $metalinclude"
						}
					} else {
...
if { [file exists $libdefs] } {
	...
	catch { exec $exec_cmd -c "cat $libdefs | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | sed -e 's/\"//g' | $macroreplace $ccsreplace $addparentpcsreplace" } ccslibdefs
	...
	if { $ccslibdefs != $refccslibdefs } {
		...
		if { $ccslibdefs != $refccslibdefs } {
			...
			if { $ccsmatch == 0 } {	
				catch { exec $exec_cmd -c "cat $libdefs | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | $macroreplace $ccsreplace $addparentpcsreplace" } firstline
				if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
					puts "SNPS_ERROR  : (PCSQA-2.1b REMOTESITE) First line of PCS lib.defs file is no INCLUDE of CCS lib\[_common\].defs: $firstline"
				} else {
					puts "SNPS_ERROR  : (PCSQA-2.1.1f REMOTESITE) First line of PCS lib.defs file is no INCLUDE of Parent PCS lib.defs: $firstline"
				}
			} else {
				if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
				
				if { $refccsmatchprojectenv == 1 } {
					puts "SNPS_ERROR  : (PCSQA-2.1n REMOTESITE) Inconsistent CCS $refccs used in PCS project.env; redefinition in PCS lib.defs"
				}

				## redefine refccs
				catch { exec $exec_cmd -c "cat $libdefs | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | sed -e 's/\"//g' | $macroreplace $ccsreplace" } ccslibdefs
	...
	puts "SNPS_ERROR  : (PCSQA-1 REMOTESITE) No lib.defs file found at $libdefs"
}
}
...
if { $siteid == "REMOTESITE" } {
	...
	if { [file exists msip_pcsqa_template] } {
		...
		catch { exec $exec_cmd -c ". $::env(MODULESHOME)/init/bash $moduleloadperforce && export P4CONFIG=\".p4config\" && $p4 sync -f hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/..." } p4message 
		if { $p4message == "hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/... - protected namespace - access denied." } {			
			## get p4 content via account csadmin to bypass restricted access
			catch { exec $exec_cmd -c "chmod -R 777 hbm3" } dummy
			...
			catch { exec bash -c "$SSH \"cd $p4root && export P4CONFIG=.p4config && /usr/local/bin/p4 sync -f hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/...\"" } p4message
			catch { exec $exec_cmd -c "chmod -R 755 hbm3" } dummy
		}	 
		catch { exec $exec_cmd -c ". $::env(MODULESHOME)/init/bash $moduleloadperforce && $p4 client -d $clientname" } dummy 
	}
	cd $run_dir
	set p4libdefs $p4root/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/design/lib.defs
}
...
proc checkp4defines { file parentfile usemetalstack } {
	...
	foreach define $defines {
		...
		if { [file isdirectory $define] == 0 } {
			incr checks
			if { [string first METAL_STACK $define] == -1 } {
				puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Refered library in $parentfile file does not exists: $define"
			} else {
				if { $usemetalstack == "all" } {
					set envtcls [glob -nocomplain $pcs/cad/*/env.tcl]
				} else {
				...
				foreach envtcl $envtcls {
					...
					incr checks
					if { [file isdirectory $metaldefine] == 0 } {
						puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Refered library in $parentfile file does not exists: $metaldefine"
					} else {
						set metaldefine  [gettargetfile $metaldefine]
						...
						if { [string compare $gpermissions "4" ] == -1 } {
							puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Refered library in $parentfile file has not at least read permission for group: $metaldefine"
						}
					}
				}
			}
			...
			incr checks
			if { [string compare $gpermissions "4" ] == -1 } {
				puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Refered library in $parentfile file has not at least read permission for group: $define"
			}
		}
	}
...
proc checkatp4depot { include parentfile } {
	if { [string first PROJ_P4_ROOT $include] != -1 || [string first MSIP_PROJ_P4WS_ROOT  $include] != -1 } {
		...
		if { [string first "no such file" $p4result] != -1 } {
			puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Include file in $parentfile file does not exists in p4: $include as $p4include in p4"
		}
	} else {
		puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Include file in $parentfile file does not exists: $include"
	}
}
...
proc checkp4includes { file parentfile usemetalstack } {
	...
	catch { exec sh -c "cat $file | grep -v ^# | grep INCLUDE | awk '{print \$2}' | $macroreplace $ccsreplace $parentpcsreplace" } includes
	set includes [split $includes \n]
		...
		if { [file exists $include] == 0 } {
			...
			if { [string first METAL_STACK $include] == -1 } {
				...
				foreach envtcl $envtcls {
					...
					if { [file exists $metalinclude] == 0 } {
						checkatp4depot $include $parentfile
						## puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Include file in $parentfile file does not exists: $metalinclude"
					} else {
						...
						incr checks
						if { [string compare $gpermissions "4" ] == -1 } {
							puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Include file in $parentfile file has not at least read permission for group: $metalinclude"
						}
						if { [lsearch -exact $filesvisited $metalinclude] == -1 } {
							lappend filesvisited $metalinclude
			...
			incr checks
			if { [string compare $gpermissions "4" ] == -1 } {
				puts "SNPS_WARNING: (PCSQA-2.2d REMOTESITE) Include file in $parentfile file has not at least read permission for group: $include"
			}
...
if { [file exists $p4libdefs] } {
	...
	catch { exec $exec_cmd -c "cat $p4libdefs | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' -e 's/\\s\\s*/ /g' | head -1" } firstline
	...
	if { $firstline != $reffirstline && $firstline != $reffirstline2 } {
		puts "SNPS_ERROR  : (PCSQA-2.1d) First line of p4 PCS design/lib.defs is not equal to: $reffirstline"	
...
foreach envtcl $envtcls {
	...
	if { [file isdirectory $parentstack] } {
		...
		catch { exec $exec_cmd -c "cat $envtcl | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | $macroreplace $ccsreplace $addparentpcsreplace | sed -e 's#CAD_METAL_STACK#$metalstack#g' -e 's#METAL_STACK#$metalstack#g'" } ccsenvtcl
		if { $ccsenvtcl != $refccsenvtcl } {
			catch { exec $exec_cmd -c "cat $envtcl | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | $macroreplace $ccsreplace $addparentpcsreplace | sed -e 's#CAD_METAL_STACK#$metalstack#g' -e 's#METAL_STACK#$metalstack#g'" } firstline
			incr checks
			if { [file exists $ccsenvtcl] } {
				...
				if { $ccsenvtcl != $refccsenvtcl } {
					...
					if { $ccsmatch == 0 } {	
						if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
							puts "SNPS_ERROR  : (PCSQA-2.1c REMOTESITE) First line of PCS cad/$metalstack/env.tcl file is no source of CCS env.tcl: $firstline"
						} else {
							puts "SNPS_ERROR  : (PCSQA-2.1.1g REMOTESITE) First line of PCS cad/$metalstack/env.tcl file is no source of Parent PCS env.tcl: $firstline"
						}
					} else {
						if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {

						if { $refccsmatchprojectenv == 1 } {
							puts "SNPS_ERROR  : (PCSQA-2.1n REMOTESITE) Inconsistent CCS $refccs used in PCS project.env; redefinition in PCS cad/$metalstack/env.tcl"
							set refccsmatchprojectenv 0
						}
						if { $refccsmatchlibdefs == 1 } {
							puts "SNPS_ERROR  : (PCSQA-2.1n REMOTESITE) Inconsistent CCS $refccs used in PCS lib.defs; redefinition in PCS cad/$metalstack/env.tcl"
							set refccsmatchlibdefs 0
						}

						## redefine refccs
						catch { exec $exec_cmd -c "cat $envtcl | grep -v ^# | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | head -1 | awk '{print \$2}' | $macroreplace $ccsreplace | sed -e 's#CAD_METAL_STACK#$metalstack#g' -e 's#METAL_STACK#$metalstack#g'" } ccsenvtcl
			...
			catch { exec $exec_cmd -c "cat $envtcl | grep -v -e ^# -e ^source | sed -e '/^\\s*$/d' -e 's/\\s\\s*$//g' | wc -l" } morecodelines
			incr checks
			if { $morecodelines > 0 } {
				puts "SNPS_WARNING: (PCSQA-2.2a REMOTESITE) There is additional code in PCS cad/$metalstack/env.tcl file other than source of CCS env.tcl"					
			}
		...
		if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
			puts "SNPS_ERROR  : (PCSQA-2.1f REMOTESITE) PCS metal stack $metalstack does not exists in CCS $refccs/cad"
		} else {
			puts "SNPS_ERROR  : (PCSQA-2.1.1g REMOTESITE) PCS metal stack $metalstack does not exists in Parent PCS $parentpcs/cad"
		}		
	}
}
}
...
proc checkcadpermissions { file } {
		...
		incr checks
		if { $cadowner != "csadmin" } {
			puts "SNPS_ERROR  : (PCSQA-2.1h REMOTESITE) $file $filetype in PCS is not owned by csadmin, but $cadowner"
		}
		set sg_pdks 65859
		catch { exec sh -c "ls -${diroption}n $targetfile | awk '{ print \$4 }'" } filegroupid
		incr checks
		if { $filegroupid != $sg_pdks } {
			puts "SNPS_ERROR  : (PCSQA-2.1h REMOTESITE) $file $filetype in PCS does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
		}
		...
		if { [matchpermissions $cadpermissions $gomw_gopr_permissions] == 0 || $gs_check == 0 } {
			...
			puts "SNPS_ERROR  : (PCSQA-2.1h REMOTESITE) $file $filetype in PCS has not $gomw_gopr_permissions_txt permissions, but $cadugopermissions"
		}
...
if { [string compare -length [string length $ude3proj] $ude3proj $pcs] == 0 } {
if { [file exists $projectenv] } {
   catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIPRunDirRestrict | awk '{ print \$3 }' | sed -e 's/\"//g'" }  msiprundirrestrictvalue
   ...
   ## Rule 2.1i is not applicable, if MSIPRunDirRestrict is defined, see rule 2.1mm
	...
	if { [file isdirectory $pcsdesign] && [file type $pcsdesign] == "link" } {
		...
		set refpcsdesign "/remote/proj/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/design"
		if { $pcsdesign != $refpcsdesign } {
			puts "SNPS_ERROR  : (PCSQA-2.1i REMOTESITE) design directory in PCS is no link to $refpcsdesign, but to $pcsdesign"
		} else {
			set pcsdesignlink 1
		}		
	}
	...
	if { [file isdirectory $pcsdesign] } {
		...
		incr checks
		if { $filegroupid != $sg_pdks } {
			puts "SNPS_ERROR  : (PCSQA-2.1i REMOTESITE) design directory in PCS does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
		}
		...
		if { [matchpermissions $designpermissions $refdesignpermissions] == 0 || $gs_check == 0 } {
			...
			puts "SNPS_ERROR  : (PCSQA-2.1i REMOTESITE) design directory in PCS has not $refdesignpermissionstxt & g+s permissions, but $designugopermissions"
		}
	}
...
if { [file exists $projectenv] } {
catch { exec $exec_cmd -c "grep 'module\[\[:space:\]\]' $projectenv | grep -v -e ^# -e '^\[\[:space:\]\]*echo' -e unload -e msip -e ude_internal_tools -e helic | wc -l" } edamodules
incr checks
if { $edamodules > 0 } {
	catch { exec $exec_cmd -c "grep 'module\[\[:space:\]\]' $projectenv | grep -v -e ^# -e '^\[\[:space:\]\]*echo' -e unload -e msip -e ude_internal_tools -e helic | awk '{print \$3}' | tr '\\n' ',' | sed -e 's/,/, /g' -e 's/, $//g'" } edamodules
	puts "SNPS_ERROR  : (PCSQA-2.1j REMOTESITE) The following EDA tools are not set by environment variable in project.env: $edamodules"
}
catch { exec $exec_cmd -c "grep setenv $projectenv | grep -v -e ^# -e unsetenv | grep MSIP_ | grep _VERSION | wc -l" } edamodules
incr checks
if { $edamodules > 0 } {
	catch { exec $exec_cmd -c "grep setenv $projectenv | grep -v -e ^# -e unsetenv | grep MSIP_ | grep _VERSION | awk '{print \$2}' | tr '\\n' ',' | sed -e 's/,/, /g' -e 's/, $//g'" } edamodules
	puts "SNPS_WARNING: (PCSQA-2.2b REMOTESITE) The following EDA tools are set in project.env with non default version: $edamodules"
}

catch { exec $exec_cmd -c "grep 'module\[\[:space:\]\]' $projectenv | grep -v ^# | grep -e load -e msip | wc -l" } msipmodules
if { $msipmodules > 0 } {

	## determine creation of PCS
	set p4port "export P4PORT=p4p-$siteid:1999"
	set pcstriple hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened
	...
	catch { exec $exec_cmd -c "grep 'module\[\[:space:\]\]' $projectenv | grep -v -e ^# -e unload | grep msip | sed -e 's#/# #g' | awk '{print \$3}'" } msipmodules
	...
	foreach msipmodule $msipmodules {
		catch { exec $exec_cmd -c "grep $msipmodule $projectenv | grep -v ^# | grep module | grep load   | grep -v unload | wc -l" } moduleloads
		catch { exec $exec_cmd -c "grep $msipmodule $projectenv | grep -v ^# | grep module | grep unload | wc -l" } moduleunloads
		incr checks
		if { $moduleloads != $moduleunloads } {
			puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) The internal tool $msipmodule is not set by module unload/load in project.env"
		}
		catch { exec $exec_cmd -c "grep $msipmodule $projectenv | grep -v ^# | grep module | grep load | grep -v unload | grep \"/\" | wc -l" } moduleversionload
		incr checks
		...
		catch { exec $exec_cmd -c "grep $msipmodule $projectenv | grep -v ^# | grep module | grep load | grep -v unload | grep -e \"/testing$\" -e \"/latest$\" -e \"/dev$\" | wc -l" } modulewrongversionload
		incr checks
	...
	if { $mandatorynoversionmoduleloaderror == 1 } {
		puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) project.env contains internal tools settings with fixed version, which should be default:$mandatorynoversionmodules"
	}
	if { $noversionmoduleloadwarning == 1 } {
		## puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) project.env contains internal tools settings with non default versions"
		## puts "SNPS_WARNING: (PCSQA-2.2b REMOTESITE) project.env contains internal tools settings with non default versions"
		puts "SNPS_WARNING: (PCSQA-2.1k REMOTESITE) project.env contains internal tools settings with default versions:$noversionmodules"
	}
	if { $wrongversionmoduleloaderror == 1 } {
		puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) project.env contains internal tools settings with testing, latest or dev versions:$wrongversionmodules"
	}
	...
	if { $creationdatesecpcs >= $sept1sec && 
		...
		foreach reftool $reftools {
			...
			if { $msipmodulesversions > 1 &&
				...
				puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $reftool is multiple loaded in project.env"
			}
			set grepreftools "$grepreftools -e $reftool"
		}		
		catch { exec $exec_cmd -c "cat $projectenv | grep '^\[\[:space:\]\]*module\[\[:space:\]\]\[\[:space:\]\]*load\[\[:space:\]\]' | grep $grepreftools | awk '{ print \$3 }' | sed -e 's#/# #g'" } msipmodulesversions
			...
			if { $msipmodule != "" && [lsearch $reftools $msipmodule] != -1 } {
				...
				if { $msipmodule == "ude_internal_tools" ||
					...
					if { [lsearch -exact $refversion($msipmodule) $msipmoduleversion] == -1 } {
						puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $msipmodule has incorrect version '$msipmoduleversion' setting instead of 'unload and default' at creation date $creationdatepcs of PCS in project.env"
					} else {
					...
					if { $msipmodule == "msip_cd_layoutMQA" &&
						...
						if { $msipmoduleversion != "" } {
							puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $msipmodule has incorrect version '$msipmoduleversion' setting instead of 'default' (after $layoutMQAdatenoversion) at creation date $creationdatepcs of PCS in project.env"
						} 
					...
					if { $msipmoduleversion == "" ||
						...
						if { $allowviolation == 0 } {
							puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $msipmodule has incorrect version '$msipmoduleversiontxt' setting instead of at least '$refversion($msipmodule)' at creation date $creationdatepcs of PCS in project.env"
						}
					}
					set available($msipmodule) 1
					...
					if { [file exists $refccs/design/project.env] } {
						...
						if { $ccsmsipmodulesversion != "" } {
							...
							if { $msipmoduleversion != "" &&
								...
								puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $msipmodule has older version '$msipmoduleversion' setting than '$ccsmsipmoduleversion' in CCS at creation date $creationdatepcs of PCS in project.env"
							}
						}					
					}
				}
		...
		foreach reftool $reftools {
			if { ( [info exists available($reftool)] == 0 ||
				...
				puts "SNPS_ERROR  : (PCSQA-2.1k REMOTESITE) $reftool has no version setting instead of '$reftoolversion' at creation date $creationdatepcs of PCS in project.env"
			}
		}
	}
}
...
if { [file exists $run_dir/${uniqueprefix}moduleprojectenv] } {
	...
	close $fidsed
	catch { exec $exec_cmd -c "grep -v ^# $projectenv | sed -f ${uniqueprefix}sedscript | grep 'module\[\[:space:\]\]' > $runscript_moduleprojectenv && rm -f ${uniqueprefix}sedscript" } dummy
}
...
if { [file size ${uniqueprefix}moduleerrors] > 0 } {
	...
	if { $moduleerrorstxt != "" } {
		...
		puts "SNPS_ERROR  : (PCSQA-2.1jk REMOTESITE) The following tools version in project.env are not available in site $siteid: $moduleerrorstxt"
	}
	...
	if { $moduleerrorstxt != "" } {
		...
		puts "SNPS_ERROR  : (PCSQA-2.1jk REMOTESITE) Tools in project.env can not be set in site $siteid due to omission of: $moduleerrorstxt"
	}
}  
}
}
...
if { $siteid != "REMOTESITE" } {
incr checks
if { [file exists $p4root/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/design/.cdesigner.tcl] } {
	puts "SNPS_ERROR  : (PCSQA-2.1m) design/.cdesigner.tcl is present in p4"
	incr p4errors
}
incr checks
if { [file exists $p4root/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/cad/.cdesigner.tcl] } {
	puts "SNPS_ERROR  : (PCSQA-2.1m) cad/.cdesigner.tcl is present in p4"
	incr p4errors
}
}
...
set automotivevalue ""
if { [file exists $projectenv] } {
	catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep automotive | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } automotivevalue
	if { $automotivevalue == "" } {
		if { $msip_parentchild_type == "CHILD_CAD_SETUP" ||
		     $msip_parentchild_type == "CHILD_ALL" } {
			catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep MSIP_PARENT_PCS_PRODUCT_NAME | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } msip_parent_pcs_product_name
			...
			if { [file exists $parentpcsprojectenv] } {
				catch { exec $exec_cmd -c "cat $parentpcsprojectenv | grep -v -e ^# -e unsetenv | grep setenv | grep automotive | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } automotivevalue
			}
		}	
	}
}
		...
		foreach rapostfix $rapostfixes {
			...
			if { [file exists $normtarget] == 0 } {
				...
				if { [file exists $parenttarget] } {			
					puts "SNPS_ERROR  : (PCSQA-2.1$emirrule("$rapostfix") REMOTESITE) cad/emir/${stackup}${rapostfix} does not exist"
				}
			} else {
				if { $emirlink != "" } {
					if { $parentpcs != "" } {
						...
						if { [string compare "$parentpcs/cad/emir/${stackup}${rapostfix}" $normtarget] != 0 } {
							puts "SNPS_ERROR  : (PCSQA-2.1$emirrule("$rapostfix") REMOTESITE) Link cad/emir/${stackup}${rapostfix} does not point to correct target: $target"
						}
					} else {
						if { $refccs != "" } {
							incr checks
...
if { $refccs != "" } {
	...
	if { $sheinfoexists == 0 } {
		puts "SNPS_ERROR  : (PCSQA-2.1aaa REMOTESITE) she.info file $pcs/cad/emir/she.info omits"
	}
}
				...
				set orgtarget $target
		     		...
		     		if { [string compare -length [string length $reftarget] $reftarget $target]     != 0 ||
				       	...
				       	switch $referencetype {
				       		"link" {
							puts "SNPS_ERROR  : (PCSQA-2.1n REMOTESITE) Link does not point to reference CCS: $src -> $orgtarget"
						}
				       		"source" -
						"include" {
							puts "SNPS_ERROR  : (PCSQA-2.1uu2 REMOTESITE) $src does not $referencetype reference CCS: $orgtarget"
						}
					}
				}
			} else {
				puts "SNPS_ERROR  : (PCSQA-2.1.1i REMOTESITE) Link does not point to Parent PCS: $src -> $target"
			}
		}
}
...
foreach link $links {
	...
	if { [file exists $normtarget] } {
		...
		puts "SNPS_ERROR  : (PCSQA-1 REMOTESITE) Broken link found at $link -> $target"		
	}
}
...
proc check_references_to_refccs_of_type { referencetype } {
	...
	foreach cadfile $cadfiles {
		catch { exec sh -c "grep -i \"^\[\[:blank:\]\]*$referencetype\[\[:blank:\]\]\" $cadfile | grep -v ^# | wc -l" } references
		if { $references > 0 } {
			catch { exec sh -c "grep -i \"^\[\[:blank:\]\]*$referencetype\[\[:blank:\]\]\" $cadfile | grep -v ^# | awk '{ print \$2 }' | $macroreplace $ccsreplace" } references
...
if { $msip_parentchild_type == "PARENT" } {
	...
	if { [file exists $projectenv] } {
		catch { exec $exec_cmd -c "cat $projectenv | grep -v -e ^# -e unsetenv | grep setenv | grep PARENT_PROJ_HOME | head -1 | awk '{print \$3}' | sed -e 's/\"//g'" } parent_proj_home
		incr checks
		if { $parent_proj_home != "" } {
			incr checks
			if { $parent_proj_home != $ref_parent_proj_home && $parent_proj_home != $ref2_parent_proj_home } {
				puts "SNPS_ERROR  : (PCSQA-2.1.1l REMOTESITE) Wrong value of setenv of PARENT_PROJ_HOME in project.env: setenv PARENT_PROJ_HOME $parent_proj_home"
			}
		} else {
			puts "SNPS_ERROR  : (PCSQA-2.1.1l REMOTESITE) No setenv of PARENT_PROJ_HOME in project.env: setenv PARENT_PROJ_HOME $ref_parent_proj_home"
		}
	}
	...
	foreach pcsmacro $pcsmacros {
		...
		foreach pcsfile $pcsfiles {
			incr checks
			catch { exec $exec_cmd -c "grep $pcsmacro $pcsfile | grep -v -e ^# -e RUN_DIR_ROOT -e CAD_PROJ_HOME -e $pcsmacrocounter | wc -l"} macropresent
			if { $macropresent > 0 } {
				puts "SNPS_ERROR  : (PCSQA-2.1.1b REMOTESITE) Use $pcsmacrocounter instead of $pcsmacro in $pcsfile"
			}
		}
	}
}
...
## 			incr checks
## 			puts "SNPS_ERROR  : (PCSQA-2.1.1i REMOTESITE) Folder does not point to Parent PCS: $linkdir"
## 		}
## 	}
## }
...
if { [file exists $pcstcl] && [file size $pcstcl] > 0 } {
	puts "SNPS_WARNING: (PCSQA-2.2c REMOTESITE) cad/pcs.tcl exists in PCS and is not empty"
}
...
if { $msip_parentchild_type == "CHILD_CAD_SETUP" && $parentpcs != "" } {
	...
	if { [file exists $pcstcl] } {
		if { [file exists $parentpcstcl] } {
			catch { exec $exec_cmd -c "cat $pcstcl | grep -v ^# | grep source | grep MSIP_PARENT_PCS_ | wc -l" } usedparentpcstcl
			if { $usedparentpcstcl > 0 } {
				catch { exec $exec_cmd -c "cat $pcstcl | grep -v ^# | grep source | grep MSIP_PARENT_PCS_ | awk '{print \$2}' | $macroreplace $parentpcsreplace" } usedparentpcstcl
				if { $usedparentpcstcl != $parentpcstcl } {
					puts "SNPS_ERROR  : (PCSQA-2.1.1h REMOTESITE) No source of Parent PCS pcs.tcl in cad/pcs.tcl in PCS: source $usedparentpcstcl"
				}
			} else {
				puts "SNPS_ERROR  : (PCSQA-2.1.1h REMOTESITE) No source of Parent PCS pcs.tcl in cad/pcs.tcl in PCS by: source \$env(MSIP_PROJ_ROOT)/\$env(MSIP_PARENT_PCS_PRODUCT_NAME)/\$env(MSIP_PARENT_PCS_PROJ_NAME)/\$env(MSIP_PARENT_PCS_REL_NAME)/cad/pcs.tcl"
			}
		}
	} else {
		if { [file exists $parentpcstcl] } {
			puts "SNPS_ERROR  : (PCSQA-2.1.1h REMOTESITE) No cad/pcs.tcl exists in PCS to source Parent PCS pcs.tcl"
		}
	}
}
...
if { ( $msip_parentchild_type == "CHILD_RnD_LIBS" || $msip_parentchild_type == "CHILD_ALL" ) && $parentpcs != "" } {
	...
	if { [file exists $designlibdefs] } {
		...
		incr checks
	       	catch { exec $exec_cmd -c "cat $designlibdefs | grep -v ^# | grep INCLUDE | grep MSIP_PARENT_PCS_ | wc -l" } usedparentdesignlibdefs
	       	if { $usedparentdesignlibdefs > 0 } {
	               catch { exec $exec_cmd -c "cat $designlibdefs | grep -v ^# | grep INCLUDE | grep MSIP_PARENT_PCS_ | awk '{print \$2}' | $macroreplace $parentpcsreplace" } usedparentdesignlibdefs
	               if { "$usedparentdesignlibdefs" != "$parentpcsp4v1/pcs/design/lib.defs"                  && "$usedparentdesignlibdefs" != "$parentpcsp4v2/pcs/design/lib.defs" &&
		            "$usedparentdesignlibdefs" != "$parentpcsp4v1/pcs/design/MSIP_METAL_STACK/lib.defs" && "$usedparentdesignlibdefs" != "$parentpcsp4v2/pcs/design/MSIP_METAL_STACK/lib.defs" } {
	        	       puts "SNPS_ERROR  : (PCSQA-2.1.1j REMOTESITE) No INCLUDE of Parent PCS design/lib.defs in design/lib.defs in PCS: INCLUDE $usedparentdesignlibdefs"
	               }
	       	} else {
	               puts "SNPS_ERROR  : (PCSQA-2.1.1j REMOTESITE) No INCLUDE of Parent PCS design/lib.defs in design/lib.defs in PCS by: INCLUDE \${MSIP_PROJ_P4WS_ROOT|PROJ_P4_ROOT}/projects/\${MSIP_PARENT_PCS_PRODUCT_NAME}/\${MSIP_PARENT_PCS_PROJ_NAME}/\${MSIP_PARENT_PCS_REL_NAME}/design/\[\${MSIP_METAL_STACK}/\]lib.defs"
	       	}
	}
}

incr checks
set pcsinprogress PCSINPROGRESS
if { $pcsinprogress == 0 } {
	catch { exec $exec_cmd -c "grep \"/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/\" $::env(udecadrep)/projects/perforce_global_pcs_template | wc -l" } pcslines
	if { $pcslines > 0 } {	
		puts "SNPS_ERROR  : (PCSQA-2.1r REMOTESITE) PCS is not 'In progress' in VC, but available in $::env(udecadrep)/projects/perforce_global_pcs_template"
	}
}
...
foreach stackuppath $stackuppaths {
	if [file exists $stackuppath/env.tcl ] {
		...
		puts $fid_sourceude "setenv MSIP_PRODUCT_NAME hbm3"
		...
		puts $fid_checkmetalstack "	puts \$fid \"SNPS_ERROR  : (PCSQA-2.1q REMOTESITE) environment variables CAD_METAL_STACK and METAL_STACK have not the same contents, CAD_METAL_STACK=\$::env(CAD_METAL_STACK) and METAL_STACK=\$::env(METAL_STACK)\""
		...
		puts $fid_checkmetalstack "		if { \[db::getNext \$lppliblibs\] == \"\" } {"
		puts $fid_checkmetalstack "			puts \$fid \"SNPS_ERROR  : (PCSQA-2.1y REMOTESITE) Libraries lpplib and lpplib2 and lpplib3 not found for metal stack $stackup\""
		puts $fid_checkmetalstack "		}"
		puts $fid_checkmetalstack "	}"	
		puts $fid_checkmetalstack "}" 
		...
		puts $fid_checkmetalstack "		puts \$fid \"SNPS_ERROR  : (PCSQA-2.1y REMOTESITE) Library \$lib not found for metal stack $stackup\""
		puts $fid_checkmetalstack "	}"
		puts $fid_checkmetalstack "}" 
...
if { $siteid != "REMOTESITE" } {
	set pcsstatus $p4root/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/cad/.PCSQA_STATUS
	incr checks
	if { [file exists $pcsstatus] == 0 } {
		puts "SNPS_ERROR  : (PCSQA-2.1t) No cad/.PCSQA_STATUS file available in p4"
		incr p4errors
	}
}
...
if { $siteid == "REMOTESITE" } {
	set pcscad $::env(PCSQA_PROJ_ROOT)/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/cad
	...
	if { [info exists ::env(MSIP_LYNX_PCSQA)] &&
		...
		if { [file exists $pcsstatus] == 0 } {
			set pcsstatus $p4root/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened/cad/$pcsqastatusfile
			if { [file exists $pcsstatus] == 0 } {
				set pcsqastatusfile ".PCSQA_STATUS"
			}
		}
	...
	if { [file exists $pcsstatus] && [file isdirectory $pcscad] } {
		...
		incr checks
		if { $chksum != $refchksum } {
		puts "SNPS_ERROR  : (PCSQA-2.1t REMOTESITE) Combined chksum of PCS files (excluding $pcsqastatusfile) (=$chksum) is not the same as the one inside $pcsqastatusfile file, PCSQA CHKSUM entry (=$refchksum)"
		...
		foreach filechksum $fileschksum {
			...
			if { $fileavailable > 0 } {
				...
				if { $actchksum != $refchksum } {
					puts "SNPS_ERROR  : (PCSQA-2.1t REMOTESITE) Chksum of file path $file (=$actchksum) is not the same as the one inside cad/$pcsqastatusfile file (=$refchksum)"
					if { [file type $file] == "link" } {
						puts "SNPS_WARNING: (PCSQA-2.1t REMOTESITE) $file is a link, the chksum difference is outside of the PCS and please check if the change is valid"
					}
				}
			} else {
				puts "SNPS_ERROR  : (PCSQA-2.1t REMOTESITE) No chksum entry in cad/$pcsqastatusfile for file path $file"
			}
			}			
		}
		}
		...
		foreach pcsstatuscadfile $pcsstatuscadfiles {
			if { $pcsstatuscadfile != "" &&
				...
				puts "SNPS_ERROR  : (PCSQA-2.1t REMOTESITE) Chksum entry in cad/$pcsqastatusfile does not exists as file path cad/$pcsstatuscadfile"
			}
		}
	...
	if { [file exists $pcs/cad] } {	
		...
		set replacepcsmacros "sed -e 's#MSIP_PRODUCT_NAME#hbm3#g' -e 's#MSIP_PROJ_NAME#d763-hbm3-v2-tsmc3eff-12-ew#g' -e 's#MSIP_REL_NAME#rel1.00_EWHardened#g'"
		...
		foreach pcsfile $pcsfiles {
			...
			foreach projvar $projvarslist {
				...
				set bberror 0
				catch { exec $exec_cmd -c "grep \"$projvar\" $pcsfile $removecommentline | $removeenvchars | grep \"$projvar/$pcstriple\" | grep -v \"^#\" | wc -l" } bbitemavail
				incr checks
				if { $bbitemavail > 0 && [iswaiveenvvarsfile $pcsfile] == 0 } {
					...
					puts "SNPS_ERROR  : (PCSQA-2.1bb REMOTESITE) Reference $projvartxt/$pcstriple in file $pcsfile, use \$PROJ_HOME" 	
				}
				...
				if { $projvaravail > 0 } {
					...
					foreach projvarref $projvarrefs {
						puts "SNPS_ERROR  : (PCSQA-2.1bb REMOTESITE) Reference '$projvarref' in file $pcsfile refers to different PCS$refccstxt"
					} 	
					set bberror 1
				}
				if { $projvar != "MSIP_PROJ_ROOT" } {
					...
					if { $ccitemavail > 0 && [iswaiveenvvarsfile $pcsfile] == 0 } {
						puts "SNPS_ERROR  : (PCSQA-2.1cc REMOTESITE) Reference $projvar in file $pcsfile, use \$MSIP_PROJ_ROOT" 	
					}
					if { $bberror == 0 } {
						catch { exec $exec_cmd -c "grep \"$projvar\" $pcsfile \
										$removecommentline | \
						...
						if { $ccitemavail > 0 } {
							...
							foreach ccitemref $ccitemrefs {
								puts "SNPS_ERROR  : (PCSQA-2.1cc REMOTESITE) Reference '$ccitemref' in file $pcsfile refers to different PCS"
							} 	
						}
					}	
				}
				...
				foreach projvar $projvars {
					...
					if { $projvaravail > 0 } {
						...
						foreach projvarref $projvarrefs {
							...
							puts "SNPS_ERROR  : (PCSQA-2.1hhh REMOTESITE) Reference '$projvarref' in file $pcsfile refers outside PCSes,CCSes,ude and fab area"
						} 	
					}
				}
						...
						foreach helicref $helicrefs {
							...
							if { $helicpath != "" } {
								...
								if { [file exists $helicpath] == 0 } {
									puts "SNPS_ERROR  : (PCSQA-2.1bb us01) Helic reference path '$helicpath' in file $pcsfile does not exist"
							     	}
							}
						}				
					}				
		...
		if { [file isdirectory $pcs/cad/shared/starrcxt] } {	
			...
			foreach starfile $starfiles {
				...
				if { $spicesubcktfileavail > 0 } {
					puts "SNPS_ERROR  : (PCSQA-2.1aa REMOTESITE) SPICE_SUBCKT_FILE available in file $starfile" 	
				}
			    }
			}
		}
	...
	set designpcs "/remote/proj/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened"
	if { [file isdirectory $designpcs] } {		
		...
		if { [file exists $projectenv] } {
			...
			if { $msiprundirrestrictvalue == "" } {
				...
				incr checks
				if { $filegroupid != $sg_pdks } {
					puts "SNPS_ERROR  : (PCSQA-2.1x REMOTESITE) PCS directory $designpcs does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
				}
			}
		}
		...
		if { [matchpermissions $designpcspermissions $gpw_permissions] == 0 } {
			...
			puts "SNPS_ERROR  : (PCSQA-2.1x REMOTESITE) PCS directory $designpcs has not $gpw_permissions_txt permissions, but $designpcsugopermissions"
		}
	    }
	}
	...
	if { [file exists $projectenv] } {
		...
		if { $p4port != $intelp4port } {
			catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e P4PORT | awk '{ print \$3 }' | sed -e 's/\"//g' -e 's/:/ /g' | awk '{ print \$2 }'" } p4port
		}
		if { $p4port != $intelp4port &&
		     $parentpcs != "" &&
		     ( $msip_parentchild_type == "CHILD_CAD_SETUP" ||
			...
			set parentpcsprojectenv $parentpcs/cad/project.env
			if { [file exists $parentpcsprojectenv] } {
				catch { exec $exec_cmd -c "cat $parentpcsprojectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e P4PORT | awk '{ print \$3 }' | sed -e 's/\"//g' -e 's/:/ /g' | awk '{ print \$2 }'" } p4port
			}
		}
		if { $p4port != $intelp4port &&
		     $refccs != "" &&
		     [file exists $refccs/design/project.env] } {
			catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e P4PORT | awk '{ print \$3 }' | sed -e 's/\"//g' -e 's/:/ /g' | awk '{ print \$2 }'" } p4port
		}
		if { $p4port == $intelp4port &&
		     $refccs != "" &&
			...
			catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep unsetenv  | grep -e MSIPRunDirRestrict | wc -l" }  pcsunsetenvmsiprundirrestrict
			if { $msiprundirrestrictvalue == "" &&
			     $parentpcs != "" &&
			     ( $msip_parentchild_type == "CHILD_CAD_SETUP" ||
			       $msip_parentchild_type == "CHILD_ALL" ) } {
				set parentpcsprojectenv $parentpcs/cad/project.env
				if { [file exists $parentpcsprojectenv] } {
					catch { exec $exec_cmd -c "cat $parentpcsprojectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIPRunDirRestrict | awk '{ print \$3 }' | sed -e 's/\"//g'" }  msiprundirrestrictvalue
					catch { exec $exec_cmd -c "cat $parentpcsprojectenv | grep -v '^#' | grep unsetenv  | grep -e MSIPRunDirRestrict | wc -l" }  parentpcsunsetenvmsiprundirrestrict
					if { $msiprundirrestrictvalue != "" } {
						## if { $pcsunsetenvmsiprundirrestrict == 0 } {
						##	puts "SNPS_ERROR  : (PCSQA-2.1mm REMOTESITE) Parent PCS sets MSIPRunDirRestrict, but PCS does not and no unsetenv"
						## }
			...
			if { $msiprundirrestrictvalue == "" &&
			     $refccs != "" &&
			     [file exists $refccs/design/project.env] } {
				catch { exec $exec_cmd -c "cat $refccs/design/project.env | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e MSIPRunDirRestrict | awk '{ print \$3 }' | sed -e 's/\"//g'" }  msiprundirrestrictvalue
				if { $msiprundirrestrictvalue != "" } {
					## if { $pcsunsetenvmsiprundirrestrict == 0 && $parentpcsunsetenvmsiprundirrestrict == 0 } {
					##	puts "SNPS_ERROR  : (PCSQA-2.1mm REMOTESITE) CCS sets MSIPRunDirRestrict, but (Parent) PCS does not and no unsetenv"
					## }
				 	set msiprundirrestrictwarning 1
			...
			if { $msiprundirrestrictvalue != "" } {
				...
				incr checks
				if { $getentresults == 0 } {
					set msgtext "(PCSQA-2.1ee1 REMOTESITE) Unix group $restrictunixgroup of MSIPRunDirRestrict value in project.env ${parentpcstext}is no valid Unix group"
				...
				if { $p4port == $intelp4port } {
					...
					if { [string equal -length 3 "icf" $restrictunixgroup ] == 0 } {
						set msgtext "(PCSQA-2.1ee1 REMOTESITE) Unix group $restrictunixgroup of MSIPRunDirRestrict value in project.env ${parentpcstext}does not start with 'icf'"
						if { $msiprundirrestrictwarning == 1 } {
							puts "SNPS_WARNING: $msgtext"
						} else {
							puts "SNPS_ERROR  : $msgtext"
				...
				if { [string length $octalcode] == 3 && $octalcode >= 0 && $octalcode <= 777 } {
					...
					if { [matchpermissions $octalcode $requiredoctalcode] == 0 } {
						set msgtext "(PCSQA-2.1ee1 REMOTESITE) Octal code $octalcode of MSIPRunDirRestrict in project.env ${parentpcstext}has not xx0 permissions"
						if { $msiprundirrestrictwarning == 1 } {
							puts "SNPS_WARNING: $msgtext"
						} else {
							puts "SNPS_ERROR  : $msgtext"
					...
					set msgtext "(PCSQA-2.1ee1 REMOTESITE) Octal code $octalcode of MSIPRunDirRestrict in project.env ${parentpcstext}is no valid Unix octal code"
					if { $msiprundirrestrictwarning == 1 } {
						puts "SNPS_WARNING: $msgtext"
					} else {
						puts "SNPS_ERROR  : $msgtext"
				...
				if { $p4port == $intelp4port } {
					...
					puts "SNPS_ERROR  : (PCSQA-2.1ee1 REMOTESITE) Omission of 'setenv MSIPRunDirRestrict <unix_group>:<octal_code>' in project.env"
				}
				incr restricterrors
			}

			if { $restricterrors == 0 && [string compare -length [string length $ude3proj] $ude3proj $pcs] == 0 } {
				if { [file type $pcs] == "link" } {
					set targetpcs [file readlink $pcs]]
					set refpcs    "/remote/proj/hbm3/d763-hbm3-v2-tsmc3eff-12-ew/rel1.00_EWHardened"
					if { $targetpcs == $refpcs } {
						incr checks
						set pcsdir $refpcs
						...
						if { $pcsdir == "/" } {
							puts "SNPS_ERROR  : (PCSQA-2.1mm REMOTESITE) Directory (or parent directory of) $refpcs has not the required MSIPRunDirRestrict settings <unix_group>:<octal_code> = $restrictunixgroup:$octalcode"
						}
					}
				}
			}
			...
			if { $refccs != "" } {
				...
				if { [file exists $envcommontcl] } {
					catch { exec $exec_cmd -c "cat $envcommontcl | grep -v '^#' | sed -e 's/db::setPrefValue\\s\\s*MSIPRunDirRestrict/\\ndb::setPrefValue MSIPRunDirRestrict/g' | grep -e \"db::setPrefValue MSIPRunDirRestrict\" | head -1 | awk '{ print \$4 }' | sed -e 's/\"//g' -e 's/\}//g'" }  ccsmsiprundirrestrictvalue
				}
			}
			if { $msiprundirrestrictvalue != "" && $ccsmsiprundirrestrictvalue != "" } {
				incr checks
			     	if { $msiprundirrestrictvalue != $ccsmsiprundirrestrictvalue } {
					puts "SNPS_ERROR  : (PCSQA-2.1ee2 REMOTESITE) MSIPRunDirRestrict values at PCS ($msiprundirrestrictvalue) in project.env and CCS ($ccsmsiprundirrestrictvalue) in env_common.tcl are different"
				}
			}
			if { $msiprundirrestrictvalue != "" } {
				set envtcls [glob -nocomplain $pcs/cad/*/env.tcl]
				...
				foreach envtcl $envtcls {
					...
					catch { exec $exec_cmd -c "cat $envtcl | grep -v '^#' | sed -e 's/db::setPrefValue\\s\\s*MSIPRunDirRestrict/\\ndb::setPrefValue MSIPRunDirRestrict/g' | grep -e \"db::setPrefValue MSIPRunDirRestrict\" | head -1 | awk '{ print \$4 }' | sed -e 's/\"//g' -e 's/\}//g'" }  metalmsiprundirrestrictvalue
					incr checks
					if { $metalmsiprundirrestrictvalue != "" } {
						incr checks
						if { $msiprundirrestrictvalue != $metalmsiprundirrestrictvalue } {
							puts "SNPS_ERROR  : (PCSQA-2.1ee2 REMOTESITE) MSIPRunDirRestrict values at PCS ($msiprundirrestrictvalue) in project.env and TCL preference value in $metalstack/env.tcl ($metalmsiprundirrestrictvalue) are different"
							set correctvalue 0
						}
					} else {
						puts "SNPS_ERROR  : (PCSQA-2.1ee2 REMOTESITE) Omission of TCL preference value setting 'db::setPrefValue MSIPRunDirRestrict -value \"$msiprundirrestrictvalue\"' in $metalstack/env.tcl"
						set omitted 1
					}
					...
					if { $ccsmsiprundirrestrictvalue == "" && $omitted == 0 && $correctvalue == 1 } {
						...
						catch { exec $exec_cmd -c "cat $envtcl | grep -v '^#' | tr -d '\n' | sed -e 's/ //g' -e 's/\\t//g' | sed -e 's/if/\\nif/g' | grep db::createPrefMSIPRunDirRestrict | sed -e 's/\}/\} /g' -e 's/\} err\\]/\}err\\]/g' -e 's/\} \{/\}\{/g' -e 's/\} /\}\\n/g' | grep db::createPrefMSIPRunDirRestrict" } actcode
						incr checks
						if { $actcode != $refcode } {
							puts "SNPS_ERROR  : (PCSQA-2.1ee2 REMOTESITE) TCL preference value of MSIPRunDirRestrict in $metalstack/env.tcl is not set by required code"
		...
		catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e PINS_VERSION | tail -1 | awk '{ print \$3 }' | sed -e 's/\"//g'" }  pins_version_value
		if { $pins_version_value == "" } {
			if { $parentpcs != "" &&
				...
				set parentpcsprojectenv $parentpcs/cad/project.env
				if { [file exists $parentpcsprojectenv] } {
					catch { exec $exec_cmd -c "cat $parentpcsprojectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e PINS_VERSION | tail -1 | awk '{ print \$3 }' | sed -e 's/\"//g'" }  pins_version_value
				}
			}	
			if { $pins_version_value == "" } {
				if { $refccs != "" } {
					set refccsprojectenv $refccs/design/project.env
					if { [file exists $refccsprojectenv] } {
						catch { exec $exec_cmd -c "cat $refccsprojectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e PINS_VERSION | tail -1 | awk '{ print \$3 }' | sed -e 's/\"//g'" }  pins_version_value
					}
				}	
			}
		}
		if { $pins_version_value != "" } {
			set pinsfile $::env(udecadrep)/msip/ude_conf/snps_pins/$pins_version_value/hbm3_pins.txt
			if { [file exists $pinsfile] == 0 } {
				set pinsfile $::env(udecadrep)/msip/ude_conf/snps_pins_gen2/production/hbm3/$pins_version_value/hbm3_pins.txt
				if { [file exists $pinsfile] == 0 } {
					puts "SNPS_WARNING: (PCSQA-2.1pp REMOTESITE) No PINS_VERSION $pins_version_value file has been found: $pinsfile"
				}	
			}
			if { $vicipinsversion != "" && $vicipinsversion != $pins_version_value } {
				puts "SNPS_ERROR  : (PCSQA-2.1pp REMOTESITE) PINS_VERSION value '$pins_version_value' in (inherited) project.env is not equal to value '$vicipinsversion' in ViCi Porting Spec"
			}			
		} else {
			puts "SNPS_ERROR  : (PCSQA-2.1pp REMOTESITE) Omission of PINS_VERSION setting in (inherited) project.env"
		}

		catch { exec $exec_cmd -c "cat $projectenv | grep -v '^#' | grep -e setenv | grep -v unsetenv | grep -e EXPORT_COMPLIANT  | awk '{ print \$3 }' | sed -e 's/\"//g'" }  export_compliant_value
		if { $export_compliant_value == 1 } {
			...
			if { $msiprundirrestrictvalue != "" } {
				...
				incr checks
				if { $getentresults == 0 } {
					puts "SNPS_ERROR  : (PCSQA-2.1qq REMOTESITE) Unix group $restrictunixgroup of MSIPRunDirRestrict value in project.env of EXPORT_COMPLIANT PCS is no valid Unix group"
				}
			} else {
				puts "SNPS_ERROR  : (PCSQA-2.1qq REMOTESITE) Omission of 'setenv MSIPRunDirRestrict <unix_group>:<octal_code>' in project.env of EXPORT_COMPLIANT PCS"
			}
			
			if { [file exists $pcs/design/lib.defs] } {
				catch { exec $exec_cmd -c "cat $pcs/design/lib.defs | grep -v -e \"^#\" -e \"^\[\[:blank:\]\]*$\" | head -2 | sed -e 's/\\s//g' -e 's/INCLUDE/INCLUDE /' > $run_dir/libdefsincludes" } dummy
				...
				if { [file size $run_dir/difflibdefsincludes] > 0 } {
					puts "SNPS_ERROR  : (PCSQA-2.1rr REMOTESITE) design/lib.defs of EXPORT_COMPLIANT PCS has not required content"
		...
		if { [file isdirectory $pcsdesign] } {
			...
			if { [string first M $diskusage] != -1 || [string first G $diskusage] != -1 } {
				...
				if { $more10M == 1 } {
					...
					puts "SNPS_ERROR  : (PCSQA-2.1$errorchars REMOTESITE) $designdir folder of PCS contains more than 10MB of information"
				}
			}
			if { $designdir == "design_unrestricted" } {
				...
				if { [file type $pcsdesign] == "link" } {
					...
					if { [string compare -length [string length $cadrep] $cadrep $targetdesign] != 0 } {
						puts "SNPS_ERROR  : (PCSQA-2.1ll REMOTESITE) $designdir folder of PCS is no link to $cadrep location, but $targetdesign"
					}
				} else {
					if { [string compare -length [string length $cadrep] $cadrep $pcsdesign] != 0 } {
						puts "SNPS_ERROR  : (PCSQA-2.1ll REMOTESITE) $designdir folder of PCS is not located in $cadrep location"
					}
				}
			}
		}
		...
		foreach modelsdir $modelsdirs {
			if { [file exists $refccs/cad/models/$modelsdir] } {
				 ...
				 if { [file exists $pcs/cad/models/$modelsdir] == 0 } {
				 	 puts "SNPS_ERROR  : (PCSQA-2.1yy REMOTESITE) Omission of $pcs/cad/models/$modelsdir"
				 } else {
				 	 if { $cvcp_has_AreaCap > 0 &&
				 		 ...
				 		 if { [string compare $targetpath $reftargetpath] != 0 } {
				 			 puts "SNPS_ERROR  : (PCSQA-2.1lll REMOTESITE) cad/models/$modelsdir does not point to $reftargetpath, but $targetcvcp"
				 		 } else {
				 			 ...
				 			 if { $targetversionint < $reftargetversionint } {
				 				 puts "SNPS_ERROR  : (PCSQA-2.1lll REMOTESITE) cad/models/$modelsdir does not point to at least $reftargetpath/$reftargetversion, but $targetversion"									 
				 			 }
				 		 }							 
				 	 }
				 }
	...
	if { [file isdirectory $pcs] && $refccs != "" } {
		...
		foreach streamlayermap $streamlayermaps {
			...
			if { [file exists $ccsstreamlayermap] } {
				...
				## catch { exec $exec_cmd -c "cat $streamlayermap | grep -v ^# | awk '{ print \$1,\$2 }'" } pcslpps
				catch { exec $exec_cmd -c "cat $streamlayermap | grep -v ^# | grep ^TEMP | awk '{ print \$1 }'" } pcslpps
...
##							}
##							if { $lppnumsequal == 0 } {
##								puts "SNPS_ERROR  : (PCSQA-2.1zz REMOTESITE) PCS lpp '$pcslpp' has different stream numbers '$pcslppnums' as in CCS '$ccslppnums', in PCS stream layermap $streamlayermap in comparison to CCS equivalent $ccsstreamlayermap"					
##							}
##						}
					}					
				}
				foreach additionalpcslpp $additionalpcslpps {
					puts "SNPS_ERROR  : (PCSQA-2.1zz REMOTESITE) Additional TEMP layer '$additionalpcslpp' in PCS stream layermap $streamlayermap in comparison to CCS equivalent $ccsstreamlayermap"					
				}				
			}
		}		
	}	
...
if { $siteid != "REMOTESITE" } {
	...
	set upcs  [string map { - _ . _ } [string toupper hbm3_d763-hbm3-v2-tsmc3eff-12-ew_rel1.00_EWHardened]]
