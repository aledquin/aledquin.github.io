...
if { $siteid == "us03" } {
	...
	if { [file isdirectory $pcsdesign] && [file type $pcsdesign] != "link" } {
		if { "us03" != "de02" && "us03" != "us01" && "us03" != "in01" } {
			puts "SNPS_WARNING: PCSQA is not applicable for site us03, because design directory in PCS is no link"
			exit 0
		} else {
			puts "SNPS_WARNING: design directory in PCS for site us03 is no link"
		}
	}
} 
...
if { [file exists $projectenv] } {
	...
	if { $msip_parentchild_type != "CHILD_RnD_LIBS" && $msip_parentchild_type != "PARENT" && $msip_parent_pcs_product_name != "" && $msip_parent_pcs_proj_name != "" && $msip_parent_pcs_rel_name != "" } {
		...
		if { $parentprojectenv != $refparentprojectenv } {
			if { $msip_parentchild_type == "CHILD_CAD_SETUP" } {
				ruleErrorApplicable "(PCSQA-2.1.1e us03) First line of PCS project.env file is not source of a Parent PCS project.env: $parentprojectenv"
			} else {
				ruleErrorApplicable "(PCSQA-2.1a us03) First line of PCS project.env file is not source of a Parent PCS project.env: $parentprojectenv"
			}
		}
	} else {
	...
	if { $ccsprojectenv != $refccsprojectenv } {
		...
		if { $ccsmatch == 0 } {
			...
			ruleErrorApplicable "(PCSQA-2.1a us03) First line of PCS project.env file is not source of CCS project.env: $firstline"
		} else {
			## redefine refccs
...
##			}
			if { $rundirrootvalue != $refvalue } {
				ruleErrorApplicable "(PCSQA-2.1v us03) Value of redefinition of RUN_DIR_ROOT in PCS project.env file is '$rundirrootvalue' and not '$refvalue'"		
			}
		} 
##		Not mandatory any more since PCS QA spec 14.1
##		else {
##			ruleErrorApplicable "(PCSQA-2.1v us03) No redefinition of RUN_DIR_ROOT in PCS project.env file is available"		
##		}
	}
	set msip_cad_rel_name  [file tail $refccs]
	set msip_cad_proj_name [file tail [file dirname $refccs]]
	...
	if { [file exists $pcs/design/project.env] } {
		...
		if { $projectenvtarget != "" } {
			...
			if { $projectenvtarget != [file normalize $projectenv] } {
				ruleErrorApplicable "(PCSQA-2.1l us03) design/project.env is no link to cad/project.env"
			}
		} else {
			ruleErrorApplicable "(PCSQA-2.1l us03) design/project.env is no link to cad/project.env"
		}		
	...
	ruleErrorApplicable "(PCSQA-1.1 us03) No project.env file found at $projectenv"
} 
...
if { [file exists $libdefs] } {
	...
	if { $ccslibdefs != $refccslibdefs } {
		...
		if { $ccslibdefs != $refccslibdefs } {
			...
			if { $ccsmatch == 0 } {	
				...
				if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
					ruleErrorApplicable "(PCSQA-2.1b us03) First line of PCS lib.defs file is no INCLUDE of CCS lib\[_common\].defs: $firstline"
				} else {
					ruleErrorApplicable "(PCSQA-2.1.1f us03) First line of PCS lib.defs file is no INCLUDE of Parent PCS lib.defs: $firstline"
				}
			} else {
				if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
				
				if { $refccsmatchprojectenv == 1 } {
					ruleErrorApplicable "(PCSQA-2.1n us03) Inconsistent CCS $refccs used in PCS project.env; redefinition in PCS lib.defs"
				}
	...
	ruleErrorApplicable "(PCSQA-1.2 us03) No lib.defs file found at $libdefs"
}
}
...
proc checkp4defines { file parentfile usemetalstack } {
	...
	foreach define $defines {
		...
		if { [file isdirectory $define] == 0 } {
			incr checks
			if { [string first METAL_STACK $define] == -1 } {
				ruleWarningApplicable "(PCSQA-2.2d us03) Refered library in $parentfile file does not exists: $define"
			} else {
				if { $usemetalstack == "all" } {
					set envtcls [glob -nocomplain $pcs/cad/*/env.tcl]
				} else {
				...
				foreach envtcl $envtcls {
					...
					incr checks
					if { [file isdirectory $metaldefine] == 0 } {
						ruleWarningApplicable "(PCSQA-2.2d us03) Refered library in $parentfile file does not exists: $metaldefine"
					} else {
						set metaldefine  [gettargetfile $metaldefine]
						...
						if { [string compare $gpermissions "4" ] == -1 } {
							ruleWarningApplicable "(PCSQA-2.2d us03) Refered library in $parentfile file has not at least read permission for group: $metaldefine"
						}
					}
				}
			}
			...
			incr checks
			if { [string compare $gpermissions "4" ] == -1 } {
				ruleWarningApplicable "(PCSQA-2.2d us03) Refered library in $parentfile file has not at least read permission for group: $define"
			}
		}
	}
...
proc checkatp4depot { include parentfile } {
	if { [string first PROJ_P4_ROOT $include] != -1 || [string first MSIP_PROJ_P4WS_ROOT  $include] != -1 } {
		...
		if { [string first "no such file" $p4result] != -1 } {
			ruleWarningApplicable "(PCSQA-2.2d us03) Include file in $parentfile file does not exists in p4: $include as $p4include in p4"
		}
	} else {
		ruleWarningApplicable "(PCSQA-2.2d us03) Include file in $parentfile file does not exists: $include"
	}
}
		...
		if { [file exists $include] == 0 } {
			...
			if { [string first METAL_STACK $include] == -1 } {
				...
				foreach envtcl $envtcls {
					...
					if { [file exists $metalinclude] == 0 } {
						checkatp4depot $include $parentfile
						## ruleWarningApplicable "(PCSQA-2.2d us03) Include file in $parentfile file does not exists: $metalinclude"
					} else {
						...
						incr checks
						if { [string compare $gpermissions "4" ] == -1 } {
							ruleWarningApplicable "(PCSQA-2.2d us03) Include file in $parentfile file has not at least read permission for group: $metalinclude"
						}
						if { [lsearch -exact $filesvisited $metalinclude] == -1 } {
							lappend filesvisited $metalinclude
			...
			incr checks
			if { [string compare $gpermissions "4" ] == -1 } {
				ruleWarningApplicable "(PCSQA-2.2d us03) Include file in $parentfile file has not at least read permission for group: $include"
			}
...
foreach envtcl $envtcls {
	...
	if { [file isdirectory $parentstack] } {
		...
		if { $ccsenvtcl != $refccsenvtcl } {
			...
			if { [file exists $ccsenvtcl] } {
				...
				if { $ccsenvtcl != $refccsenvtcl } {
					...
					if { $ccsmatch == 0 } {	
						if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
							ruleErrorApplicable "(PCSQA-2.1c us03) First line of PCS cad/$metalstack/env.tcl file is no source of CCS env.tcl: $firstline"
						} else {
							ruleErrorApplicable "(PCSQA-2.1.1g us03) First line of PCS cad/$metalstack/env.tcl file is no source of Parent PCS env.tcl: $firstline"
						}
					} else {
						if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {

						if { $refccsmatchprojectenv == 1 } {
							ruleErrorApplicable "(PCSQA-2.1n us03) Inconsistent CCS $refccs used in PCS project.env; redefinition in PCS cad/$metalstack/env.tcl"
							set refccsmatchprojectenv 0
						}
						if { $refccsmatchlibdefs == 1 } {
							ruleErrorApplicable "(PCSQA-2.1n us03) Inconsistent CCS $refccs used in PCS lib.defs; redefinition in PCS cad/$metalstack/env.tcl"
							set refccsmatchlibdefs 0
						}
			...
			incr checks
			if { $morecodelines > 0 } {
				ruleWarningApplicable "(PCSQA-2.2a us03) There is additional code in PCS cad/$metalstack/env.tcl file other than source of CCS env.tcl"					
			}
		...
		if { $msip_parentchild_type != "CHILD_CAD_SETUP" && $msip_parentchild_type != "CHILD_ALL" } {
			ruleErrorApplicable "(PCSQA-2.1f us03) PCS metal stack $metalstack does not exists in CCS $refccs/cad"
		} else {
			ruleErrorApplicable "(PCSQA-2.1.1g us03) PCS metal stack $metalstack does not exists in Parent PCS $parentpcs/cad"
		}		
	}
}
}
...
proc checkcadpermissions { file } {
		...
		incr checks
		if { $cadowner != "csadmin" } {
			ruleErrorApplicable "(PCSQA-2.1h us03) $file $filetype in PCS is not owned by csadmin, but $cadowner"
		}
		set sg_pdks 65859
		catch { exec sh -c "ls -${diroption}n $targetfile | awk '{ print \$4 }'" } filegroupid
		incr checks
		if { $filegroupid != $sg_pdks } {
			ruleErrorApplicable "(PCSQA-2.1h us03) $file $filetype in PCS does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
		}
		...
		if { [matchpermissions $cadpermissions $gomw_gopr_permissions] == 0 || $gs_check == 0 } {
			...
			ruleErrorApplicable "(PCSQA-2.1h us03) $file $filetype in PCS has not $gomw_gopr_permissions_txt permissions, but $cadugopermissions"
		}
...
if { [file exists $projectenv] } {
   ...
   if { $msiprundirrestrictvalue != "" && 
	...
	ruleErrorApplicable "(PCSQA-2.1jj us03) No design_unrestricted folder, despite of MSIPRunDirRestrict restriction in project.env"	
   }
   ...
   if { $msiprundirrestrictvalue == "" &&
	...
	if { [file isdirectory $pcsdesign] && [file type $pcsdesign] == "link" } {
		...
		if { $pcsdesign != $refpcsdesign } {
			ruleErrorApplicable "(PCSQA-2.1i us03) design directory in PCS is no link to $refpcsdesign, but to $pcsdesign"
		} else {
			set pcsdesignlink 1
		}		
	}
	...
	if { [file isdirectory $pcsdesign] } {
		...
		incr checks
		if { $filegroupid != $sg_pdks } {
			ruleErrorApplicable "(PCSQA-2.1i us03) design directory in PCS does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
		}
		...
		if { [matchpermissions $designpermissions $refdesignpermissions] == 0 || $gs_check == 0 } {
			...
			ruleErrorApplicable "(PCSQA-2.1i us03) design directory in PCS has not $refdesignpermissionstxt & g+s permissions, but $designugopermissions"
		}
	}
...
if { $edamodules > 0 } {
	...
	ruleErrorApplicable "(PCSQA-2.1j us03) The following EDA tools are not set by environment variable in project.env: $edamodules"
}
...
if { $edamodules > 0 } {
	...
	ruleWarningApplicable "(PCSQA-2.2b us03) The following EDA tools are set in project.env with non default version: $edamodules"
}
...
if { $msipmodules > 0 } {
	...
	foreach msipmodule $msipmodules {
		...
		if { $moduleloads != $moduleunloads } {
			ruleErrorApplicable "(PCSQA-2.1k us03) The internal tool $msipmodule is not set by module unload/load in project.env"
		}
	...
	if { $mandatorynoversionmoduleloaderror == 1 } {
		ruleErrorApplicable "(PCSQA-2.1k us03) project.env contains internal tools settings with fixed version, which should be default:$mandatorynoversionmodules"
	}
	if { $noversionmoduleloadwarning == 1 } {
		## ruleErrorApplicable "(PCSQA-2.1k us03) project.env contains internal tools settings with non default versions"
		## ruleWarningApplicable "(PCSQA-2.2b us03) project.env contains internal tools settings with non default versions"
		ruleWarningApplicable "(PCSQA-2.1k us03) project.env contains internal tools settings with default versions:$noversionmodules"
	}
	if { $wrongversionmoduleloaderror == 1 } {
		ruleErrorApplicable "(PCSQA-2.1k us03) project.env contains internal tools settings with testing, latest or dev versions:$wrongversionmodules"
	}
	...
	if { $creationdatesecpcs >= $sept1sec && 
		...
		foreach reftool $reftools {
			...
			if { $msipmodulesversions > 1 &&
				...
				ruleErrorApplicable "(PCSQA-2.1k us03) $reftool is multiple loaded in project.env"
			}
			...
			if { $msipmodule != "" && [lsearch $reftools $msipmodule] != -1 } {
				...
				if { $msipmodule == "ude_internal_tools" ||
					...
					if { [lsearch -exact $refversion($msipmodule) $msipmoduleversion] == -1 } {
						ruleErrorApplicable "(PCSQA-2.1k us03) $msipmodule has incorrect version '$msipmoduleversion' setting instead of 'unload and default' at creation date $creationdatepcs of PCS in ${parentprojectenvtxt}project.env"
					} else {
					...
					if { $msipmodule == "msip_cd_layoutMQA" &&
						...
						if { $msipmoduleversion != "" } {
							ruleErrorApplicable "(PCSQA-2.1k us03) $msipmodule has incorrect version '$msipmoduleversion' setting instead of 'default' (after $layoutMQAdatenoversion) at creation date $creationdatepcs of PCS in ${parentprojectenvtxt}project.env"
						} 
					...
					if { $msipmoduleversion == "" ||
						...
						if { $allowviolation == 0 } {
							ruleErrorApplicable "(PCSQA-2.1k us03) $msipmodule has incorrect version '$msipmoduleversiontxt' setting instead of at least '$refversion($msipmodule)' at creation date $creationdatepcs of PCS in ${parentprojectenvtxt}project.env"
						}
					}
					set available($msipmodule) 1
					...
					if { [file exists $refccs/design/project.env] } {
						...
						if { $ccsmsipmodulesversion != "" } {
							...
							if { $msipmoduleversion != "" &&
								...
								ruleErrorApplicable "(PCSQA-2.1k us03) $msipmodule has older version '$msipmoduleversion' setting than '$ccsmsipmoduleversion' in CCS at creation date $creationdatepcs of PCS in ${parentprojectenvtxt}project.env"
							}
						}					
					}
				}
		...
		foreach reftool $reftools {
			if { ( [info exists available($reftool)] == 0 ||
				...
				ruleErrorApplicable "(PCSQA-2.1k us03) $reftool has no version setting instead of '$reftoolversion' at creation date $creationdatepcs of PCS in ${parentprojectenvtxt}project.env"
			}
		}
	}
}
...
if { [file size ${uniqueprefix}moduleerrors] > 0 } {
	...
	if { $moduleerrorstxt != "" } {
		...
		ruleErrorApplicable "(PCSQA-2.1jk us03) The following tools version in project.env are not available in site $siteid: $moduleerrorstxt"
	}
	...
	if { $moduleerrorstxt != "" } {
		...
		ruleErrorApplicable "(PCSQA-2.1jk us03) Tools in project.env can not be set in site $siteid due to omission of: $moduleerrorstxt"
	}
}  
}
}
		...
		foreach rapostfix $rapostfixes {
			...
			if { [file exists $normtarget] == 0 } {
				...
				if { [file exists $parenttarget] } {			
					ruleErrorApplicable "(PCSQA-2.1$emirrule("$rapostfix") us03) cad/emir/${stackup}${rapostfix} does not exist"
				}
			} else {
				if { $emirlink != "" } {
					if { $parentpcs != "" } {
						...
						if { [string compare "$parentpcs/cad/emir/${stackup}${rapostfix}" $normtarget] != 0 } {
							ruleErrorApplicable "(PCSQA-2.1$emirrule("$rapostfix") us03) Link cad/emir/${stackup}${rapostfix} does not point to correct target: $target"
						}
					} else {
						if { $refccs != "" } {
							incr checks
...
if { $refccs != "" } {
	...
	if { $sheinfoexists == 0 } {
		ruleErrorApplicable "(PCSQA-2.1aaa us03) she.info file $pcs/cad/emir/she.info omits"
	}
}
				...
				set orgtarget $target
		     		...
		     		if { [string compare -length [string length $reftarget] $reftarget $target]     != 0 ||
				       	...
				       	switch $referencetype {
				       		"link" {
							ruleErrorApplicable "(PCSQA-2.1n us03) Link does not point to reference CCS: $src -> $orgtarget"
						}
				       		"source" -
						"include" {
							ruleErrorApplicable "(PCSQA-2.1uu2 us03) $src does not $referencetype reference CCS: $orgtarget"
						}
					}
				}
			} else {
				ruleErrorApplicable "(PCSQA-2.1.1i us03) Link does not point to Parent PCS: $src -> $target"
			}
		}
}
...
foreach link $links {
	...
	if { [file exists $normtarget] } {
		...
		ruleErrorApplicable "(PCSQA-1.3 us03) Broken link found at $link -> $target"		
	}
}
...
if { $msip_parentchild_type == "PARENT" } {
	...
	if { [file exists $projectenv] } {
		...
		if { $parent_proj_home != "" } {
			...
			if { $parent_proj_home != $ref_parent_proj_home && $parent_proj_home != $ref2_parent_proj_home } {
				ruleErrorApplicable "(PCSQA-2.1.1l us03) Wrong value of setenv of PARENT_PROJ_HOME in project.env: setenv PARENT_PROJ_HOME $parent_proj_home"
			}
		} else {
			ruleErrorApplicable "(PCSQA-2.1.1l us03) No setenv of PARENT_PROJ_HOME in project.env: setenv PARENT_PROJ_HOME $ref_parent_proj_home"
		}
	}
	...
	foreach pcsmacro $pcsmacros {
		...
		foreach pcsfile $pcsfiles {
			...
			if { $macropresent > 0 } {
				ruleErrorApplicable "(PCSQA-2.1.1b us03) Use $pcsmacrocounter instead of $pcsmacro in $pcsfile"
			}
		}
	}
}
...
## 			incr checks
## 			ruleErrorApplicable "(PCSQA-2.1.1i us03) Folder does not point to Parent PCS: $linkdir"
## 		}
## 	}
## }
...
if { [file exists $pcstcl] && [file size $pcstcl] > 0 } {
	ruleWarningApplicable "(PCSQA-2.2c us03) cad/pcs.tcl exists in PCS and is not empty"
}
		...
		if { [file exists $parentpcstcl] } {
			...
			if { $usedparentpcstcl > 0 } {
				...
				if { $usedparentpcstcl != $parentpcstcl } {
					ruleErrorApplicable "(PCSQA-2.1.1h us03) No source of Parent PCS pcs.tcl in cad/pcs.tcl in PCS: source $usedparentpcstcl"
				}
			} else {
				ruleErrorApplicable "(PCSQA-2.1.1h us03) No source of Parent PCS pcs.tcl in cad/pcs.tcl in PCS by: source \$env(MSIP_PROJ_ROOT)/\$env(MSIP_PARENT_PCS_PRODUCT_NAME)/\$env(MSIP_PARENT_PCS_PROJ_NAME)/\$env(MSIP_PARENT_PCS_REL_NAME)/cad/pcs.tcl"
			}
		}
	} else {
		if { [file exists $parentpcstcl] } {
			ruleErrorApplicable "(PCSQA-2.1.1h us03) No cad/pcs.tcl exists in PCS to source Parent PCS pcs.tcl"
		}
	}
}
	       	...
	       	if { $usedparentdesignlibdefs > 0 } {
	               ...
	               if { "$usedparentdesignlibdefs" != "$parentpcsp4v1/pcs/design/lib.defs"                  && "$usedparentdesignlibdefs" != "$parentpcsp4v2/pcs/design/lib.defs" &&
		            "$usedparentdesignlibdefs" != "$parentpcsp4v1/pcs/design/MSIP_METAL_STACK/lib.defs" && "$usedparentdesignlibdefs" != "$parentpcsp4v2/pcs/design/MSIP_METAL_STACK/lib.defs" } {
	        	       ruleErrorApplicable "(PCSQA-2.1.1j us03) No INCLUDE of Parent PCS design/lib.defs in design/lib.defs in PCS: INCLUDE $usedparentdesignlibdefs"
	               }
	       	} else {
	               ruleErrorApplicable "(PCSQA-2.1.1j us03) No INCLUDE of Parent PCS design/lib.defs in design/lib.defs in PCS by: INCLUDE \${MSIP_PROJ_P4WS_ROOT|PROJ_P4_ROOT}/projects/\${MSIP_PARENT_PCS_PRODUCT_NAME}/\${MSIP_PARENT_PCS_PROJ_NAME}/\${MSIP_PARENT_PCS_REL_NAME}/design/\[\${MSIP_METAL_STACK}/\]lib.defs"
	       	}
	}
}
...
if { $pcsinprogress == 0 } {
	...
	if { $pcslines > 0 } {	
		ruleErrorApplicable "(PCSQA-2.1r us03) PCS is not 'In progress' in VC, but available in $::env(udecadrep)/projects/perforce_global_pcs_template"
	}
}
...
if { $siteid == "us03" } {
	...
	if { [file exists $pcsstatus] && [file isdirectory $pcscad] } {
		...
		incr checks
		if { $chksum != $refchksum } {
		ruleErrorApplicable "(PCSQA-2.1t us03) Combined chksum of PCS files (excluding $pcsqastatusfile) (=$chksum) is not the same as the one inside $pcsqastatusfile file, PCSQA CHKSUM entry (=$refchksum)"
		...
		foreach filechksum $fileschksum {
			...
			if { $fileavailable > 0 } {
				...
				if { $actchksum != $refchksum } {
					ruleErrorApplicable "(PCSQA-2.1t us03) Chksum of file path $file (=$actchksum) is not the same as the one inside cad/$pcsqastatusfile file (=$refchksum)"
					if { [file type $file] == "link" } {
						ruleWarningApplicable "(PCSQA-2.1t us03) $file is a link, the chksum difference is outside of the PCS and please check if the change is valid"
					}
				}
			} else {
				ruleErrorApplicable "(PCSQA-2.1t us03) No chksum entry in cad/$pcsqastatusfile for file path $file"
			}
			}			
		}
		}
		...
		foreach pcsstatuscadfile $pcsstatuscadfiles {
			if { $pcsstatuscadfile != "" &&
				...
				ruleErrorApplicable "(PCSQA-2.1t us03) Chksum entry in cad/$pcsqastatusfile does not exists as file path cad/$pcsstatuscadfile"
			}
		}
		...
		foreach pcsfile $pcsfiles {
			...
			foreach projvar $projvarslist {
				...
				if { $bbitemavail > 0 && [iswaiveenvvarsfile $pcsfile] == 0 } {
					...
					ruleErrorApplicable "(PCSQA-2.1bb us03) Reference $projvartxt/$pcstriple in file $pcsfile, use \$PROJ_HOME" 	
				}
				...
				if { $projvaravail > 0 } {
					...
					foreach projvarref $projvarrefs {
						ruleErrorApplicable "(PCSQA-2.1bb us03) Reference '$projvarref' in file $pcsfile refers to different PCS$refccstxt"
					} 	
					set bberror 1
				}
				if { $projvar != "MSIP_PROJ_ROOT" } {
					...
					if { $ccitemavail > 0 && [iswaiveenvvarsfile $pcsfile] == 0 } {
						ruleErrorApplicable "(PCSQA-2.1cc us03) Reference $projvar in file $pcsfile, use \$MSIP_PROJ_ROOT" 	
					}
					if { $bberror == 0 } {
						catch { exec $exec_cmd -c "grep \"$projvar\" $pcsfile \
										$removecommentline | \
						...
						if { $ccitemavail > 0 } {
							...
							foreach ccitemref $ccitemrefs {
								ruleErrorApplicable "(PCSQA-2.1cc us03) Reference '$ccitemref' in file $pcsfile refers to different PCS"
							} 	
						}
					}	
				}
				...
				foreach projvar $projvars {
					...
					if { $projvaravail > 0 } {
						...
						foreach projvarref $projvarrefs {
							...
							ruleErrorApplicable "(PCSQA-2.1hhh us03) Reference '$projvarref' in file $pcsfile refers outside PCSes,CCSes,ude and fab area"
						} 	
					}
				}
		...
		if { [file isdirectory $pcs/cad/shared/starrcxt] } {	
			...
			foreach starfile $starfiles {
				...
				if { $spicesubcktfileavail > 0 } {
					ruleErrorApplicable "(PCSQA-2.1aa us03) SPICE_SUBCKT_FILE available in file $starfile" 	
				}
			    }
			}
		}
	...
	if { [file isdirectory $designpcs] } {		
		...
		if { [file exists $projectenv] } {
			...
			if { $msiprundirrestrictvalue == "" } {
				...
				incr checks
				if { $filegroupid != $sg_pdks } {
					ruleErrorApplicable "(PCSQA-2.1x us03) PCS directory $designpcs does not belong to $sg_pdks (sg_pdks) group ID, but $filegroupid"
				}
			}
		}
		...
		if { [matchpermissions $designpcspermissions $gpw_permissions] == 0 } {
			...
			ruleErrorApplicable "(PCSQA-2.1x us03) PCS directory $designpcs has not $gpw_permissions_txt permissions, but $designpcsugopermissions"
		}
	    }
	}
	...
	if { [file exists $projectenv] } {
			...
			if { [file exists $parentpcsprojectenv] } {
				...
				if { $msiprundirrestrictvalue != "" } {
					if { $pcsunsetenvmsiprundirrestrict == 0 } {
						ruleErrorApplicable "(PCSQA-2.1mm us03) Parent PCS sets MSIPRunDirRestrict, but PCS does not and no unsetenv"
					}
		...
		if { $msiprundirrestrictvalue == "" &&
			...
			if { $msiprundirrestrictvalue != "" } {
				if { $pcsunsetenvmsiprundirrestrict == 0 &&
					...
					ruleErrorApplicable "(PCSQA-2.1mm us03) CCS sets MSIPRunDirRestrict, but (Parent) PCS does not and no unsetenv"
				}
			 	set msiprundirrestrictwarning 1
		...
		if { $msiprundirrestrictvalue != "" } {
			...
			incr checks
			if { $getentresults == 0 } {
				set msgtext "(PCSQA-2.1$rule us03) Unix group $restrictunixgroup of MSIPRunDirRestrict value in project.env ${parentpcstext}is no valid Unix group"
			...
			if { [string length $octalcode] == 3 && $octalcode >= 0 && $octalcode <= 777 } {
				...
				if { [matchpermissions $octalcode $requiredoctalcode] == 0 } {
					set msgtext "(PCSQA-2.1$rule us03) Octal code $octalcode of MSIPRunDirRestrict in project.env ${parentpcstext}has not xx0 permissions"
					if { $msiprundirrestrictwarning == 1 } {
						ruleWarningApplicable "$msgtext"
					} else {
						ruleErrorApplicable "$msgtext"
				...
				set msgtext "(PCSQA-2.1$rule us03) Octal code $octalcode of MSIPRunDirRestrict in project.env ${parentpcstext}is no valid Unix octal code"
				if { $msiprundirrestrictwarning == 1 } {
					ruleWarningApplicable "$msgtext"
				} else {
					ruleErrorApplicable "$msgtext"
			...
			if { $intelpcs } {
				...
				ruleErrorApplicable "(PCSQA-2.1ee1 us03) Omission of 'setenv MSIPRunDirRestrict <unix_group>:<octal_code>' in project.env"
			}
			incr restricterrors
		}
		...
		if { $restricterrors == 0 &&
			...
			if { [file type $pcs] == "link" } {
				...
				if { $targetpcs == $refpcs } {
					...
					if { $pcsdir == "/" } {
						ruleErrorApplicable "(PCSQA-2.1mm us03) Directory (or parent directory of) $refpcs has not the required MSIPRunDirRestrict settings <unix_group>:<octal_code> = $restrictunixgroup:$octalcode"
					}
				}
			}
		}
		...
		if { $msiprundirrestrictvalue != "" && $ccsmsiprundirrestrictvalue != "" } {
			incr checks
		     	if { $msiprundirrestrictvalue != $ccsmsiprundirrestrictvalue } {
				ruleErrorApplicable "(PCSQA-2.1ee2 us03) MSIPRunDirRestrict values at PCS ($msiprundirrestrictvalue) in project.env and CCS ($ccsmsiprundirrestrictvalue) in env_common.tcl are different"
			}
		}
		if { $msiprundirrestrictvalue != "" } {
			set envtcls [glob -nocomplain $pcs/cad/*/env.tcl]
			...
			foreach envtcl $envtcls {
				...
				if { $metalmsiprundirrestrictvalue != "" } {
					...
					if { $msiprundirrestrictvalue != $metalmsiprundirrestrictvalue } {
						ruleErrorApplicable "(PCSQA-2.1ee2 us03) MSIPRunDirRestrict values at PCS ($msiprundirrestrictvalue) in project.env and TCL preference value in $metalstack/env.tcl ($metalmsiprundirrestrictvalue) are different"
						set correctvalue 0
					}
				} else {
					ruleErrorApplicable "(PCSQA-2.1ee2 us03) Omission of TCL preference value setting 'db::setPrefValue MSIPRunDirRestrict -value \"$msiprundirrestrictvalue\"' in $metalstack/env.tcl"
					set omitted 1
				}
				...
				if { $ccsmsiprundirrestrictvalue == "" && $omitted == 0 && $correctvalue == 1 } {
					...
					incr checks
					if { $actcode != $refcode } {
						ruleErrorApplicable "(PCSQA-2.1ee2 us03) TCL preference value of MSIPRunDirRestrict in $metalstack/env.tcl is not set by required code"
		...
		if { $pins_version_value != "" } {
			...
			if { [file exists $pinsfile] == 0 } {
				...
				if { [file exists $pinsfile] == 0 } {
					ruleWarningApplicable "(PCSQA-2.1pp us03) No PINS_VERSION $pins_version_value file has been found: $pinsfile"
				}	
			}
			if { $vicipinsversion != "" && $vicipinsversion != $pins_version_value } {
				ruleErrorApplicable "(PCSQA-2.1pp us03) PINS_VERSION value '$pins_version_value' in (inherited) project.env is not equal to value '$vicipinsversion' in ViCi Porting Spec"
			}			
		} else {
			ruleErrorApplicable "(PCSQA-2.1pp us03) Omission of PINS_VERSION setting in (inherited) project.env"
		}
		...
		if { $export_compliant_value == 1 } {
			...
			if { $msiprundirrestrictvalue != "" } {
				...
				incr checks
				if { $getentresults == 0 } {
					ruleErrorApplicable "(PCSQA-2.1qq us03) Unix group $restrictunixgroup of MSIPRunDirRestrict value in project.env of EXPORT_COMPLIANT PCS is no valid Unix group"
				}
			} else {
				ruleErrorApplicable "(PCSQA-2.1qq us03) Omission of 'setenv MSIPRunDirRestrict <unix_group>:<octal_code>' in project.env of EXPORT_COMPLIANT PCS"
			}
			...
			if { [file exists $pcs/design/lib.defs] } {
				...
				if { [file size $run_dir/difflibdefsincludes] > 0 } {
					ruleErrorApplicable "(PCSQA-2.1rr us03) design/lib.defs of EXPORT_COMPLIANT PCS has not required content"
		...
		if { [file isdirectory $pcsdesign] } {
			...
			if { [string first M $diskusage] != -1 || [string first G $diskusage] != -1 } {
				...
				if { $more10M == 1 } {
					...
					ruleErrorApplicable "(PCSQA-2.1$errorchars us03) $designdir folder of PCS contains more than 10MB of information"
				}
			}
			if { $designdir == "design_unrestricted" } {
				...
				if { [file type $pcsdesign] == "link" } {
					...
					if { [string compare -length [string length $cadrep] $cadrep $targetdesign] != 0 } {
						ruleErrorApplicable "(PCSQA-2.1ll us03) $designdir folder of PCS is no link to $cadrep location, but $targetdesign"
					}
				} else {
					if { [string compare -length [string length $cadrep] $cadrep $pcsdesign] != 0 } {
						ruleErrorApplicable "(PCSQA-2.1ll us03) $designdir folder of PCS is not located in $cadrep location"
					}
				}
			}
		}
		...
		foreach modelsdir $modelsdirs {
			if { [file exists $refccs/cad/models/$modelsdir] } {
				 ...
				 if { [file exists $pcs/cad/models/$modelsdir] == 0 } {
				 	 ruleErrorApplicable "(PCSQA-2.1yy us03) Omission of $pcs/cad/models/$modelsdir"
				 } else {
				 	 if { $cvcp_has_AreaCap > 0 &&
				 		 ...
				 		 if { [string compare $targetpath $reftargetpath] != 0 } {
				 			 ruleErrorApplicable "(PCSQA-2.1lll us03) cad/models/$modelsdir does not point to $reftargetpath, but $targetcvcp"
				 		 } else {
				 			 ...
				 			 if { $targetversionint < $reftargetversionint } {
				 				 ruleErrorApplicable "(PCSQA-2.1lll us03) cad/models/$modelsdir does not point to at least $reftargetpath/$reftargetversion, but $targetversion"									 
				 			 }
				 		 }							 
				 	 }
				 }
...
##							}
##							if { $lppnumsequal == 0 } {
##								ruleErrorApplicable "(PCSQA-2.1zz us03) PCS lpp '$pcslpp' has different stream numbers '$pcslppnums' as in CCS '$ccslppnums', in PCS stream layermap $streamlayermap in comparison to CCS equivalent $ccsstreamlayermap"					
##							}
##						}
					}					
				}
				foreach additionalpcslpp $additionalpcslpps {
					ruleErrorApplicable "(PCSQA-2.1zz us03) Additional TEMP layer '$additionalpcslpp' in PCS stream layermap $streamlayermap in comparison to CCS equivalent $ccsstreamlayermap"					
				}				
			}
		}		
	}	
...
if { "ddr54" == "std" &&
	...
	foreach envtcl $envtcls {
		...
		if { [file isdirectory $pcs/cad/$metalstack/lltech] == 0 } {
			puts "SNPS_ERROR  : (PCSQA-2.1.3a us03) No directory cad/$metalstack/lltech exists"					
		} else {
			if { 0 && [file exists $pcs/cad/$metalstack/lltech/env.tcl] == 0 } {
				## 2.1.3g
				## spec 26.0
				puts "SNPS_ERROR  : (PCSQA-2.1.3g us03) No cad/$metalstack/lltech/env.tcl exists"					
			}
			...
			if { 0 && $completed_refccs } {
				...
				foreach ccslltechcontentpath $ccslltechcontentpaths {
					...
					if { [file exists $pcs/cad/$metalstack/lltech/$ccslltechcontent] == 0 ||
						...
						puts "SNPS_ERROR  : (PCSQA-2.1.3b us03) No link cad/$metalstack/lltech/$ccslltechcontent exists"					
					} else {
						set ccstarget [gettargetfile $pcs/cad/$metalstack/lltech/$ccslltechcontent]
						incr checks
						if { $ccstarget != [file normalize $ccslltechcontentpath] } {
							puts "SNPS_ERROR  : (PCSQA-2.1.3b us03) Link cad/$metalstack/lltech/$ccslltechcontent does not link to $ccslltechcontentpath"					
						}
					}
				}
			}
	...
	if { [file exists $pcs/usage.txt] == 0 } {
		puts "SNPS_ERROR  : (PCSQA-2.1.3c us03) No $pcs/usage.txt exists"						
	}
	...
	if { $node != "" && $node <= 7 } {
		...
		if { [file exists $libdefs] == 0 } {
			puts "SNPS_ERROR  : (PCSQA-2.1.3d us03) No design/lib.defs exists"						
		} else {
			...
			incr checks
			if { $includefound == 0 } {
				puts "SNPS_ERROR  : (PCSQA-2.1.3d us03) No INCLUDE of cad/<metal-stack>/lltech/layout_tech/lib.defs exists in design/lib.defs"						
			}			
		}	
	}
	...
	if { $node != "" && $node > 7 } {
		incr checks
		if { [file exists $libdefs] == 0 } {
			puts "SNPS_ERROR  : (PCSQA-2.1.3e us03) No design/lib.defs exists"						
		} else {
			catch { exec sh -c "cat $libdefs | grep -v ^\[\[:blank:\]\]*# | grep INCLUDE     | \
			                                   grep -v -e PROJ_P4_ROOT -e SOFTINCLUDE | \
							   awk '{print \$2}' | sed -e 's#MSIP_PROJ_ROOT#$::env(PCSQA_PROJ_ROOT)#g' | \
			...
			incr checks
			if { $includefound == 0 } {
				puts "SNPS_ERROR  : (PCSQA-2.1.3e us03) No INCLUDE of $refccs/design/lib.defs exists in design/lib.defs"						
			}			
		}		
	}
	...
	if { [file exists $pcs/cad/shared/options_dmy] == 0 ||
		...
		puts "SNPS_ERROR  : (PCSQA-2.1.3f us03) No empty $pcs/cad/shared/options_dmy exists"						
	}
	...
	foreach envtcl $envtcls {
		...
		if { [file exists $lltechenvtcl] } {
			...
			if { $path == "" ||
			    ( $refpath != "" &&
			      "$path" != "$refpath" ) } {
				puts "SNPS_ERROR  : (PCSQA-$rule us03) No define of stream layer map by $refpath in cad/$metalstack/lltech/env.tcl"						
			}
		}
	}
